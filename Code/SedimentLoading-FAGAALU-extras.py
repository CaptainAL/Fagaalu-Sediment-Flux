...
def plotManningCalibrate_k(show=False,log=False,save=False): ## Rating Curves
    ...
    LBJ_S, LBJ_n, LBJ_k = 0.016, 'Jarrett', arange(0.1,1,.2)
    for k in LBJ_k:
        print str(k)
        LBJ_Man_Jarrett = Mannings_Series(datadir+'Q/Cross_Section_Surveys/LBJ_cross_section.xlsx','LBJ_m',Slope=LBJ_S,Manning_n=LBJ_n,k=k,stage_series=LBJ_Manstage)
        LBJ_Man_Jarrett['Q'] = LBJ_Man_Jarrett['Q']*1000        
        site_lbj.plot(LBJ_Man_Jarrett['Q'],LBJ_Manstage,'-',markersize=2,label='k='+str(k))
        site_lbj_zoom.plot(LBJ_Man_Jarrett['Q'],LBJ_Manstage,'-',markersize=2,label='k='+str(k))
    ...
    savefig(save,title)
    return
plotManningCalibrate_k(show=True,log=False,save=False)

### Calibrate Mannings n kfactor at LBJ
def plotManningCalibrate_n(show=False,log=False,save=False): ## Rating Curves
    ...
    ## LBJ Mannings from stream survey
    LBJ_Manstage = LBJ_Man_reduced['stage']*100
    LBJ_S, LBJ_n, LBJ_k = 0.016, np.append(arange(0.035,.13,.02),array(.07)), 1
    for n in LBJ_n:
        print str(n)
