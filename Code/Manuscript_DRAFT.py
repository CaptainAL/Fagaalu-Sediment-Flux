...
document.add_paragraph("Faga'alu watershed can be divided into two subwatersheds: 1) an upper subwatershed characterized by large areas of undisturbed, steeply-sloping, heavily forested hillsides (UPPER), and 2) a lower subwatershed wtih similarly steep forested topography and relatively small flat areas that are urbanized or densely settled (LOWER) (Figure "+Study_Area_map['fig_num']+"). This settlement pattern is typical for volcanic islands with steep topography in the South Pacific. The Faga'alu watershed includes two unique features not found in "+'"typical"'+" watersheds in American Samoa: 1) an open aggregate quarry in the  LOWER_QUARRY subwatershed, upstream of FG2 in Figure "+Study_Area_map['fig_num']+"), and 2) a large impervious area associated with a hospital in the LOWER_VILLAGE subwatershed (adjacent FG3 in Figure "+Study_Area_map['fig_num']+"). To separate these key features the LOWER subwatershed can be further divided into a LOWER_QUARRY subwatershed, draining areas between FG1 and FG2, and a LOWER_VILLAGE subwatershed, draining areas between FG2 and FG3. Three water impoundment structures were built in the UPPER subwatershed for drinking water supply and hydropower but only the highest, Matafao Reservoir, was ever connected to the municipal water system and has since fallen out of use (Tonkin & Taylor International Ltd., 1989)(Figure "+Study_Area_map['fig_num']+"). A full description of stream impoundments is in the Appendix. ")
...
document.add_paragraph("A main objective for this study was to determine how much human disturbance has increased total SSY to Faga'alu Bay (SSYTOTAL). Two approaches were used to determine relative contributions to SSYTOTAL from the undisturbed and human-disturbed areas: comparing individual event and average percent contributions, and the Disturbance Ratio (DR). SSY from the UPPER subwatershed (SSYUPPER) was measured at FG1, SSYTOTAL was measured at FG3, and SSY from the LOWER subwatershed (SSYLOWER) was calculated by subtracting SSYUPPER from SSYTOTAL (SSYLOWER=SSYFG3-SSYFG1) (Figure "+Study_Area_map['fig_num']+"). SSYLOWER includes contributions from the quarry and village areas, so where SSYEV data at FG2 were also available, the contributions from the LOWER_QUARRY and LOWER_VILLAGE subwatersheds could be determined separately. SSY from the LOWER_QUARRY subwatershed was calculated by subtracting SSYUPPER from SSY measured at FG2 (SSYLOWER_QUARRY=SSYFG2-SSYFG1), and SSY from LOWER_VILLAGE subwatershed was calculated by subtracting SSY from the UPPER and LOWER_QUARRY subwatersheds from SSYTOTAL (SSYLOWER_VILLAGE=SSYFG3-SSYFG2). The percent contributions were calculated for each storm event and averaged to determine the relative contributions from the UPPER and LOWER subwatersheds to total SSY. The DR was calculated from the average SSY contribution from the subwatershed.") 
...
document.add_paragraph("The CampbellSci OBS500 measures both BS and SS, and both the BS-SSC and SS-SSC relationships at FG3 showed high r2 values (Figure "+LBJ_OBSa_Rating_Curve['fig_num']+" and Figure "+LBJ_OBSb_Rating_Curve['fig_num']+"). Mean SS was used since it is physically measured the same as NTU measured by the YSI turbidimeter (Chauncey W. Anderson 2005).") 

## LBJ OBSa T-SSC rating curve
if 'LBJ_OBSa_Rating_Curve' in locals():
    document.add_picture(LBJ_OBSa_Rating_Curve['filename']+'.png',width=Inches(6))
    add_figure_caption(LBJ_OBSa_Rating_Curve['fig_num'],"Turbidity-Suspended Sediment Concentration relationships for the OBS500 turbidimeter deployed at FG3 ("+"{:%m/%d/%Y}".format(LBJ_OBSa.dropna().index[0])+"-"+"{:%m/%d/%Y}".format(LBJ_OBSa.dropna().index[-1])+").")

## LBJ OBSb T-SSC rating curve
if 'LBJ_OBSb_Rating_Curve' in locals():
    document.add_picture(LBJ_OBSb_Rating_Curve['filename']+'.png',width=Inches(6))
    add_figure_caption(LBJ_OBSb_Rating_Curve['fig_num'],"Turbidity-Suspended Sediment Concentration relationships for the OBS500 turbidimeter deployed at FG3 ("+"{:%m/%d/%Y}".format(LBJ_OBSb.dropna().index[0])+"-"+"{:%m/%d/%Y}".format(LBJ_OBSb.dropna().index[-1])+").")
...
document.add_paragraph("The correlation coefficients for precipitation and discharge metrics are more similar for the Total watershed than for the UPPER subwatershed. For the TOTAL watershed, correlation coefficients for Psum are nearly equal to those for Qsum. However, for the UPPER subwatershed, the discharge metrics show much higher Pearson and Spearman correlation coefficients than the precipitation metrics (Spearman's for Qsum: "+"%.2f"%QsumS_upper.spearman+" vs. Psum: "+"%.2f"%PS_upper.spearman+"). This suggests that sediment production processes are more related to discharge processes in the UPPER subwatershed, and more related to precipitation processes in the LOWER subwatershed, influencing the correlation coefficients for the TOTAL watershed. SSY from the LOWER subwatershed is hypothesized to be mostly generated by surface erosion at the quarry, dirt roads, and agricultural plots, whereas SSY from the UPPER subwatershed is hypothesized to be mainly from channel processes and mass wasting. Mass wasting can contribute large pulses of sediment during large precipitation events, which can be deposited in lobes on the streambanks and entrained at high discharges during later events. Given the high correlation coefficients for Qmax in both subwatersheds, this suggests Qmax may be a promising predictor that integrates both precipitation and discharge processes.")
## Best SSYEV model
