...
#### Quarry_picture
Quarry_picture = {'filename':maindir+'Figures/Quarry_Mitigation/2_Quarry before retention ponds.tif','fig_num':str(fig_count())}
...
## DR = SSY/SSYPRE
DR_eq = Equations[3].table
DR_eq.eq_num = eq_count()

## predict_SSYev = aXb
predict_SSYEV_eq = Equations[4].table
...
p.add_run(" P"); p.add_run("smeas").font.subscript = True
p.add_run(" P"); p.add_run("sann").font.subscript = True
...
### Data Collection
document.add_heading('Field Data Collection Methods',level=3)
...
document.add_paragraph("Precipitation (P) was measured at three locations in Faga'alu watershed using Rainwise RAINEW tipping-bucket rain gages (RG1 and RG2) and a Vantage Pro Weather Station (Wx) (Figure "+Study_Area_map['fig_num']+"). Data at RG2 was only recorded January-March, 2012, to determine a relationship between elevation and precipitation in the LOWER subwatershed. While previous data suggest that precipitation increases with elevation, here we do not calculate watershed-mean precipitation, and instead use precipitation depth at RG1 to indicate the depth of rainfall during a storm event. Most sheetwash and rill erosion, which depends on rainfall intensity and erosivity, occurred at the quarry, near the location of RG1. Rainfall data from RG1 is therefore most representative of rainfall at the quarry. The total event precipitation (Psum) and event Erosivity Index (EI30) were calculated using data from RG1, with data gaps filled by 15 min interval precipitation data from Wx.")
...
document.add_paragraph("Uncertainty in SSYEV estimates arises from both measurement and model errors, including stage-Q and T-SSC (Harmel et al., 2006). The Root Mean Square Error (RMSE) method estimates the "+'"most probable value"'+" of the cumulative or combined error by propagating the error from each measurement and modeling procedure to the final SSYEV calculation (Topping, 1972). The resulting cumulative probable error (PE) is the square root of the sum of the squares of the maximum values of the separate errors:")

add_equation(PE_eq) ## Equation

document.add_paragraph("EQmeas and ESSCmeas were estimated using lookup tables from the DUET-H/WQ software tool (Harmel et al., 2009). The effect of uncertain SSYEV estimates may complicate conclusions about contributions from subwatersheds, anthropogenic impacts, and SSYEV-Storm Metric relationships. This is common in sediment yield studies where successful models estimate SSY with "u"\u00B1""50-100% accuracy (Duvert et al., 2012) but the difference in SSY from undisturbed and disturbed areas was expected to be much larger than the cumulative uncertainty. PE was calculated for SSYEV from the UPPER and TOTAL watersheds, but not calculated for SSYEV from the LOWER subwatershed since it was calculated as the difference of SSYUPPER and SSYTOTAL.") 
...
document.add_paragraph("The disturbance ratio (DR) is the ratio of SSYEV from the watershed under current conditions to SSY under pre-disturbance conditions, estimated using sSSYUPPER:")
...
document.add_paragraph("Annual estimates of SSY and sSSY were used to compare Faga'alu with other literature. A continuous annual time-series of SSY was not possible at the study site due to the discontinuous field campaigns and failure of or damage to the turbidimeters during some months. Continuous records of P and Q were available for 2014, so the Psum-SSYEV and Qmax-SSYEV models (Equation "+predict_SSYEV_eq.eq_num+") were used to predict SSY for all storms in 2014 (Basher et al., 1997). Sediment mitigation structures were installed at the quarry in October 2014, greatly reducing SSY from the LOWER_QUARRY subwatershed (unpublished data), so the Qmax-SSY relationship developed prior to the mitigation was used to calculate the annual pre-mitigation sediment yield. For storms with no Qmax data at FG3, Qmax was predicted from a linear regression between Qmax at FG1 and Qmax at FG3 for the study period (not shown).")
...
document.add_paragraph("Using the storm definition criteria, "+"%.0f"%No_of_Storm_Intervals+" events were identified from Q data at FG1 and FG3 between January, 2012, and July 2014; "+str(len(Q_budget)-1)+" events had simultaneous Q data at FG1 and FG3 (Appendix 3, Table "+Q_budget_table.table_num+"). SSC data from T or interpolated grab samples were recorded during "+"%.0f"%No_of_Storm_Intervals_DAM_S+" events at FG1, and "+"%.0f"%No_of_Storm_Intervals_LBJ_S+" events at FG3. Of those storms, "+S_budget['Storm Start']['Total/Avg']+" events had data for P, Q, and SSC at both FG1 and FG3 to calculate SSY from the LOWER subwatershed. SSY data from interpolated grab samples were collected at FG2 for "+No_of_Storm_Intervals_QUA_S+" storms to calculate SSY from the LOWER_QUARRY and LOWER_VILLAGE subwatersheds separately. Storm event durations ranged from "+"%.0f"%All_Storms['duration (hrs)'].min()+" hour to "+"%.0f"%max_storm_duration+" days, with mean duration of "+"%.0f"%All_Storms['duration (hrs)'].mean()+" hours.")
...
H1, KWp1 = "%.3f"%H1, "%.3f"%KWp1 
H2, KWp2 = "%.3f"%H2, "%.3f"%KWp2
...
document.add_paragraph("Probability plots of the SSC data collected at FG1, FG2 and FG3 showed they were highly non-normal, so non-parametric tests for statistical significance were applied. The results of the Kruskall-Wallis test were significant for non-stormflow (p="+KWp1+") and stormflow (p="+KWp2+"); means of SSC samples were significantly different among all three locations. The results of the pair-wise Mann-Whitney test between FG1 and FG2 were significant (non-stormflow, p="+QUARRY_DAM_mannwhit1_p+"; stormflow, p="+QUARRY_DAM_mannwhit2_p+"), but between FG2 and FG3 were not significant  (non-stormflow, p="+QUARRY_LBJ_mannwhit1_p+"; stormflow, p="+QUARRY_LBJ_mannwhit2_p+"); means of SSC samples at FG1 and FG2 were significantly different, but FG2 and FG3 were not.")
...
document.add_paragraph("Given the close proximity of the quarry to the stream, SSC downstream of the quarry can be highly influenced by mining activity like rock extraction, crushing, and/or hauling operations. During 2012, a common practice for removing fine sediment from crushed aggregate was to rinse it with water pumped from the stream. In the absence of retention structures the fine sediment was then discharged directly into the stream, causing high SSC during baseflow periods with no precipitation in the preceding 24 hours (solid symbols, Figure "+Discharge_Concentration['fig_num']+"b-c). Riverine discharge of fine sediment rinsed from aggregate was discontinued in 2013. In 2013 and 2014, waste sediment was piled on-site and severe erosion of these changing stockpiles caused high SSC during storm events.")


#### Cumulative Probable Error
document.add_heading('Cumulative Probable Error (PE)',level=4)
## Most important first SSYEV error
document.add_paragraph("Cumulative Probable Error (RMSE %) for SSY estimates at FG1 and FG3 were calculated from the measurement errors for Q (8.5%) and SSC grab samples (16.3%), and the model errors of the respective stage-Q and T-SSC relationships for that location. Cumulative Probable Errors (PE) in SSYEV were "+"%.0f"%S_budget['UPPER PE %'].min()+"-"+"%.0f"%S_budget['UPPER PE %'][:-3].max()+"% ("u"\u03BC""="+S_budget['UPPER PE %']['Total/Avg']+"%) at FG1 and "+"%.0f"%S_budget['TOTAL PE %'].min()+"-"+"%.0f"%S_budget['TOTAL PE %'][:-3].max()+"% ("u"\u03BC""="+S_budget['TOTAL PE %']['Total/Avg']+"%) at FG3. ")
## PE for Water Discharge Q
document.add_paragraph("The measurement error (RMSE) for Q at FG1 and FG3 was "+"%.1f"%AV_Q_measurement_RMSE+" %, which included error in the area-velocity measurements (6%), continuous Q measurement in a natural channel (6%), pressure transducer error (0.1%), and streambed condition (firm, stable bed=0%)(DUET-H/WQ look-up table (Harmel et al., 2006)). The model errors (RMSE) were "+"%.0f"%LBJ_Man_rmse+"% for the stage-Q rating curve using Manning's equation at FG3, and "+"%.0f"%DAM_HEC_rmse+"% using HEC-RAS at FG1.")
## PE for SSC and SSY
document.add_paragraph("The measurement error (RMSE) for SSC was 16.3%, which included errors for sample collection and analysis. Sample collection error consisted of interpolating over a 30 min interval (5%) and sampling during stormflows (3%). Sample analsysis error was from measuring SSC by filtration (3.9%). The model errors (RMSE) of the T-SSC relationships were "+"%.0f"%DAM_YSI_rating_rmse+"% ("+"%.0f"%T_SSC_DAM_YSI[0].rmse+" mg/L) for the YSI and TS turbidimeters at FG1, "+"%.0f"%LBJ_YSI_rating_rmse+"% ("+"%.0f"%T_SSC_LBJ_YSI[0].rmse+" mg/L) for the YSI turbidimeter at FG3, and "+"%.0f"%LBJ_OBS_rating_rmse+"% ("+"%.0f"%T_SSC_LBJ_OBS[0].rmse+" mg/L) for the OBS turbidimeter at FG3.")
...
document.add_paragraph("SSYEV was measured simultaneously at FG1 and FG3 for "+S_budget['Storm Start']['Total/Avg']+" storms (Table "+S_budget_table.table_num+"). SSYTOTAL was "+SSY_TOTAL_2+u"\u00B1"+"%.1f"%PE_SSY_TOTAL_2+" tons ("+sSSY_TOTAL_2+u"\u00B1"+"%.1f"%PE_sSSY_TOTAL_2+" tons/km"u"\u00B2""), with "+SSY_UPPER_2+u"\u00B1"+"%.1f"%PE_SSY_UPPER_2+" tons ("+sSSY_UPPER_2+u"\u00B1"+"%.1f"%PE_sSSY_UPPER_2+" tons/km"u"\u00B2"") from the UPPER subwatershed and "+SSY_LOWER_2+" tons ("+sSSY_LOWER_2+" tons/km"u"\u00B2"") from the LOWER subwatershed. The UPPER and LOWER subwatersheds are similar in size (0.90 km"u"\u00B2"" and 0.88 km"u"\u00B2"") but SSYUPPER accounted for an average of just "+Percent_Upper_S_2 +"% and SSYLOWER for "+Percent_Lower_S_2 +"% of SSY TOTAL at the watershed outlet (Table "+S_budget_table.table_num+"). The DR estimated from sSSYUPPER (="+sSSY_UPPER_2+" tons/km"u"\u00B2"") and sSSYLOWER (Equation "+DR_eq.eq_num+") suggests sSSY has increased by "+S_budget['LOWER tons']['DR']+"x in the LOWER subwatershed, and "+S_budget['TOTAL tons']['DR']+"x for the TOTAL watershed.")


document.add_paragraph("SSY from the undisturbed forest areas in the LOWER watershed was estimated to be "+S_budget_analysis['LOWER']['Forested areas']+" tons (Equation "+SSY_disturbed.eq_num+"), so SSY from the disturbed areas was "+S_budget_analysis['LOWER']['Disturbed areas']+" tons (Table "+S_budget_analysis_table.table_num+"). For the storms in Table "+S_budget_table.table_num+", roughly "+S_budget_analysis['LOWER']['percent from disturbed areas']+"% of SSYLOWER was from disturbed areas, despite the disturbed areas only accounting for "+S_budget_analysis['LOWER']['Fraction of subwatershed area disturbed']+"% of the LOWER subwatershed area (0.089 km"u"\u00B2""). Similarly, despite only "+S_budget_analysis['TOTAL']['Fraction of subwatershed area disturbed']+"% of the TOTAL watershed being disturbed, SSY from disturbed areas accounted for "+S_budget_analysis['TOTAL']['percent from disturbed areas']+"% of the SSYTOTAL. sSSY from disturbed areas in the LOWER subwatershed was "+"{:,g}".format(float(S_budget_analysis['LOWER']['sSSY, disturbed areas (tons/km2)']))+" tons/km"u"\u00B2"", or "+S_budget_analysis['LOWER']['DR for sSSY from disturbed areas']+"x the sSSY of undisturbed forest.")



## Storm Sediment Budget Table: UPPER, LOWER, LOWER_QUARRY, LOWER_VILLAGE, TOTAL
document.add_paragraph("It was hypothesized the quarry was a key sediment source, but SSYEV was measured simultaneously at FG1, FG2, and FG3 for only "+No_of_Storm_Intervals_QUA_S+" of the storms in Table "+S_budget_table.table_num+". SSYEV was calculated separately from the LOWER subwatershed containing the quarry (SSYLOWER_QUARRY) and LOWER subwatershed containing the village below the quarry (SSYLOWER_VILLAGE) for those "+No_of_Storm_Intervals_QUA_S+" storms (Table "+S_budget_2_table.table_num+"). For the "+No_of_Storm_Intervals_QUA_S+" storms in Table "+S_budget_2_table.table_num+", SSYTOTAL was "+SSY_TOTAL_4+" tons with an average of "+Percent_UPPER_S_4+"% from the UPPER subwatershed, "+Percent_QUARRY_S+"% from the LOWER_QUARRY subwatershed, and "+Percent_VILLAGE_S+"% from the LOWER_VILLAGE subwatershed. sSSY from the UPPER, LOWER_QUARRY, and LOWER_VILLAGE subwatersheds, and the TOTAL watershed was "+sSSY_UPPER_4+", "+sSSY_LOWER_QUARRY+", "+sSSY_LOWER_VILLAGE+", and "+sSSY_TOTAL_4+" tons/km"u"\u00B2"", respectively. The results from the smaller sample of storms in Table "+S_budget_2_table.table_num+", show a slightly lower increase in SSY from the TOTAL watershed, "+S_budget_2['TOTAL tons']['DR']+"x, but show the sSSY has increased by "+S_budget_2['LOWER_QUARRY tons']['DR']+"x in the LOWER_QUARRY subwatershed and "+S_budget_2['LOWER_VILLAGE tons']['DR']+"x in the LOWER_VILLAGE subwatershed.")
...
document.add_paragraph("SSYEV from the UPPER and TOTAL watersheds was correlated with each of the four storm metrics tested (Figure "+SSY_models_ALL['fig_num']+"), though the correlations with precipitation metrics were poor in the heavily forested UPPER watershed (Table "+All_Models_stats_table.table_num+"). Pearson and Spearman correlation coefficients were similar, meaning the relationships were mostly linear in log-log space. Significant scatter was observed around all models, which reflects the changing sediment availability at the quarry and village, and the natural variability in the watershed response for different storm events.")

document.add_paragraph("Qsum was the best predictor of SSYEV for the UPPER watershed, and Psum was the best predictor for the TOTAL watershed, though Qmax was nearly as good a predictor for both watersheds. SSYEV is calculated from Q so it is expected that Qsum should be more closely correlated, and this has also been observed in other studies (Duvert et al., 2010; Rankl, 2004). Indeed the Qsum model for the UPPER watershed showed the highest coefficient of determination (r"u"\u00B2""), lowest RMSE, and highest Pearson and Spearman correlation coefficients (Table "+All_Models_stats_table.table_num+"). Psum showed an equally high r"u"\u00B2"" and a lower RMSE, but only for the TOTAL subwatershed. This suggests that sediment production is more related to discharge processes in the UPPER subwatershed, and more related to precipitation processes in the LOWER subwatershed. However, discharge metrics were also highly correlated with SSYEV in the TOTAL watershed, suggesting discharge metrics are good predictors in both disturbed and undisturbed watersheds. Qmax was not the best predictor in either watershed, but performed well in both watersheds, with similar correlation statistics to both Qsum and Psum models. Most of the scatter in the Qmax-SSYEV relationship is observed for small events, and Qmax correlated strongly with the largest SSYEV values when most of the annual sediment load is generated. ")

document.add_paragraph("Precipitation was measured at the quarry, which may reflect precipitation characteristics more accurately in the LOWER than the UPPER watershed. SSY from the LOWER subwatershed is hypothesized to be mostly generated by hillslope erosion by sheetwash and rill formation at the quarry and on dirt roads, and agricultural plots, whereas SSY from the UPPER subwatershed is hypothesized to be mainly from channel processes and mass wasting. Mass wasting can contribute large pulses of sediment which can be deposited near or in the streams and entrained at high discharges during later storm events. Given the high correlation coefficients between SSYEV and Qmax in both watersheds, Qmax may be a promising predictor that integrates both precipitation and discharge processes.")



if 'SSY_models_ALL' in locals():
    document.add_picture(SSY_models_ALL['filename']+'.png',width=Inches(6))
    add_figure_caption(SSY_models_ALL['fig_num'],"SSYEV regression models for predictive storm metrics. Each point represents a different storm event. **=slopes and intercepts were statistically different (p<0.01), *=intercepts were statistically different (p<0.01).")

document.add_paragraph("In all models, SSYEV from the TOTAL watershed was higher than from the UPPER watershed for the full range of measured storms with the exception of a few events that are considered outliers. These events could be attributed to measurement error or to landslides in the UPPER subwatershed and the increased sediment supply for that specific event. The separation of multi-peak storm events, storm sequence, and antecedent conditions may also play a role. While the climate on Tutuila is tropical, without strong seasonality, periods of low rainfall can persist for several weeks, perhaps altering the water and sediment dynamics in the subsequent storm events. ")

## Storm size
document.add_paragraph("ANCOVA was used to compare regression coefficients ("u"\u03B2""=slope and "u"\u03B1""=intercept) of the UPPER and TOTAL SSYEV models, to determine if the relative sediment contribution from undisturbed and human-disturbed areas changed with storm size. All model intercepts were significantly different (p<0.01), but only the Qsum-SSYEV model showed significantly different slopes (p<0.01). We hypothesized that for large storms, SSYEV from the UPPER watershed may become relatively more important for SSY at the outlet, however, the models show conflicting results. The Qsum-SSYEV models indicate that for larger storm events SSYEV from the UPPER and TOTAL watersheds are more similar, as the regression lines converge at higher Qsum values. Conversely, the Psum- and Qmax-SSYEV models show no change in relative contributions of SSY over the range of storm sizes (Figure "+SSY_models_ALL['fig_num']+").")
...
## From Psum- and Qmax-SSYEV relationships
document.add_paragraph("The Psum- and Qmax-SSYEV relationships were used to predict SSYEV from Psum and Qmax for "+str(len(no_storms_2014))+" storms in 2014, the only year with a continuous Q record (Table "+est_Annual_SSY_table.table_num+"). Annual SSY (sSSY) from the UPPER and TOTAL watersheds, predicted by Psum, was "+SSY_Upper_Psum_2014+" tons/year ("+sSSY_Upper_Psum_2014+" tons/km"u"\u00B2""/year) and "+SSY_Total_Psum_2014+" tons/year ("+sSSY_Total_Psum_2014+" tons/km"u"\u00B2""/year), respectively. Annual SSY predicted from Qmax was much higher. Annual SSY (sSSY) in 2014 from the UPPER and TOTAL watersheds, predicted by Qmax,  was "+SSY_Upper_Qmax_2014+" tons/year ("+sSSY_Upper_Qmax_2014+" tons/km"u"\u00B2""/year) and "+SSY_Total_Qmax_filled_2014+" tons/year ("+sSSY_Total_Qmax_filled_2014+" tons/km"u"\u00B2""/year), respectively. The Psum model is likely not a reliable predictor for the UPPER watershed, and underpredicted SSY from the TOTAL watershed for large events. It is presented here as an example of a model that was characterized by fairly strong correlation statistics (for the TOTAL watershed at least), but provided a poor annual SSY estimate. ")
...
document.add_paragraph("Human disturbance has increased sediment yield to Faga'alu Bay by "+S_budget['TOTAL tons']['DR']+"x over pre-disturbance levels. The human-disturbed subwatershed accounted for the majority ("+S_budget['% LOWER']['Total/Avg']+"%) of total sediment yield, and the quarry (1.1% of watershed area) contributed almost half of total SSY to the Bay. Qmax was a good predictor of SSYEV in both the disturbed and undisturbed watershed, making it a promising predictor in diverse environments. The slopes of the Qmax-SSYEV relationships were comparable with other studies, but the model intercepts were an order of magnitude lower than intercepts from watersheds in semi-arid to semi-humid climates. This suggests that sediment availability is relatively low in the Faga'alu watershed, either because of the heavy forest cover or volcanic rock type. The relative contribution from the human-disturbed watershed was hypothesized to diminish with increasing storm size but the results from precipitation metrics and discharge metrics were contradictory. The Qsum-SSYEV model showed that the relative contribution of SSYEV from the human-disturbed watershed decreases with storm size, but the Psum- and Qmax-SSYEV models show no change in relative contributions over increasing storm size. It was hypothesized that SSY from natural areas would become the dominant source for larger storm events, but the DR remains high for large storm events due to the naturally low SSY from natural forest areas in Faga'alu watershed. This suggests that disturbed areas were not supply limited for the range of sampled storms. The event-wise approach was advantageous because it did not require continuous in situ monitoring for a single or multiple years, which would not have been logistically possible in this remote study area. This study presents an innovative method to combine sampling and analysis strategies to measure sediment contributions from key sources, estimate baseline annual sediment yields prior to management, and rapidly develop an empirical sediment yield model for a remote, data-poor watershed. ")
