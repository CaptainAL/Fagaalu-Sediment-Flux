...
document.add_paragraph("Most events showed a typical pattern of a short period of intense rainfall that causes a rapid increase in SSC below the QUARRY, indicating rapid sheetwash of sedimnent into the stream. Highest event SSC was typically observed at QUARRY, with slightly lower and later peak SSC observed at VILLAGE. SSC at FOREST typically increased slower, remained much lower, and peaked later than the downstream sites. Though peak SSC was highest at QUARRY, the total SSY was highest at VILLAGE due to the addition of storm runoff from the relatively larger watershed downstream that includes both storm runoff from disturbed areas of the quarry and village, and the undisturbed forest on the steep hillsides (Figure "+Example_Storm['fig_num']+").")
if 'Example_Storm' in locals():
    document.add_picture(Example_Storm['filename']+'.png',width=Inches(6))
    add_figure_caption(Example_Storm['fig_num'],"Example of storm event ("+"{:%m/%d/%Y}".format(LBJ_StormIntervals.loc[63]['start'])+"). ")
    
document.add_paragraph("Overall, "+"%.0f"%No_of_Storm_Intervals+" storm events were identified from discharge data from January, 2012, to July 2014 using the stage threshold method, and manual separation of complex storm events. Valid discharge data was recorded during "+"%.0f"%No_of_Storm_Intervals_DAM_Q+" events at FOREST (upstream site), and "+"%.0f"%No_of_Storm_Intervals_LBJ_Q+" events at VILLAGE (downstream site). Valid SSC data from T and Interpolated Grab samples was recorded during "+"%.0f"%No_of_Storm_Intervals_DAM_S+" events at FOREST, and "+"%.0f"%No_of_Storm_Intervals_LBJ_S+" events at VILLAGE. Of those storms, "+"%.0f"%len(S_Diff_table)+" events had valid P and SSY data for both the FOREST and VILLAGE to calculate and compare SSY from the UPPER and LOWER watersheds (Table "+S_Diff_table.table_num+").")
document.add_paragraph("Storm event durations ranged from "+"%.0f"%LBJ_StormIntervals['duration (hrs)'].min()+" hours to "+"%.0f"%+max_storm_duration+" days, with mean duration of "+"%.0f"%LBJ_StormIntervals['duration (hrs)'].mean()+" hours.") 
