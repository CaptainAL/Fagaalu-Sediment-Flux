...
def add_figure_caption(fig_num=str(len(document.inline_shapes)),caption=''):
    ...
    manuscript_cap.paragraph_format.first_line_indent = Inches(0.5)
...
document.add_paragraph("SSYEV, m3, km2, SSYUPPER, sSSYUPPER, SSYLOWER, sSSYLOWER, SSYTOTAL, sSSYUPPER, alpha, SSYFG3, SSYFG2, SSYFG1, SSYLOWER_QUARRY, SSYLOWER_VILLAGE, Beta, plus-minus, r2, SSYdisturbed, SSYsubwatershed, Areaundist, EQmeas, ESSCmeas, mu")
...
abstract = document.add_paragraph("Anthropogenic watershed disturbance by deforestation, mining, agriculture, and urbanization often increases fluvial sediment yields, enhancing sediment stress on ecosystems near the outlets of impacted watersheds. Suspended sediment yields (SSY) during baseflow and storm events from undisturbed and human-disturbed portions of a small (1.8 km2), steep, tropical watershed that drains to a sediment-stressed coral reef were measured . Data on precipitation (P), water discharge (Q), turbidity (T), and suspended sediment concentration (SSC) were collected downstream of key sediment sources to calculate event-wise SSY (SSYEV) for 64 storms. A sediment budget and sediment rating curves were used to quantify the contribution of human-disturbed areas to SSYEV. SSC was significantly higher downstream of the quarry during base- and stormflows. The human-disturbed watershed accounted for 86% of SSYEV on average, and a 3.6x increase over natural sediment loading. Specific SSY (tons/area) from the quarry was over 120x higher than natural forest, and the quarry, which covers 5% of the watershed area, contributed nearly 51% of SSYEV. Similar to mountainous watersheds in semi-arid and temperate watersheds, SSYEV from both the undisturbed and disturbed watersheds had the highest correlation with event maximum discharge (Qmax, Pearson's R=0.89 for both watersheds) compared with precipitation, event Q, and an erosivity index. Annual sediment yield estimates varied from 29-70 tons/yr (tons/km2/yr) from the undisturbed subwatershed, and 341-450 tons/yr (tons/km2/yr) from the human-disturbed subwatershed, depending on the estimation method. Only 10% of the watershed is disturbed by humans but sediment yield has increased significantly (3.6x). Identification of hotspots like the quarry will help mitigation efforts.")
...
document.add_paragraph("The suspended sediment load (SSY) was calculated at three sampling points in Faga'alu stream that drain key land covers suspected of having different SSY: FG1 drains undisturbed forest in the UPPER subwatershed, FG2 drains undisturbed forest and the quarry in the LOWER_QUARRY subwatershed, and FG3 drains undisturbed forest and the village in the LOWER_VILLAGE subwatershed. While steep, mountainous watersheds can discharge large amounts of bedload (Milliman and Syvitski, 1992), this research is focused on sediment size fractions that can be transported in suspension in the marine environment to settle on corals; this is generally restricted to silt and clay fractions (<16um) (Asselman, 2000).")
...
document.add_paragraph("Differences in the power law parameters for undisturbed and disturbed subwatersheds (Lewis et al., 2001) were tested using Analysis of Covariance (ANCOVA). A difference in intercept would indicate higher sediment discharge from the human-disturbed subwatershed for the same size storm event. A difference in slope would indicate the relative sediment contribution from the human-disturbed subwatershed changes with increasing storm size. If regression slopes for the UPPER and TOTAL watersheds are significantly different, it supports the conclusion that the effect of human-disturbance changes with storm size.")
...
document.add_paragraph("Continuous discharge and precipitation data showed approximately 60% of annual precipitation fell during storms that exceeded the stage threshold, and 40% of precipitation fell during low-intensity events that did not exceed the defined storm threshold. Annual storm precipitation is then determed to be 60% of measured annual precipitation. Considering most SSY is discharged during a few, relatively large events, it is assumed that small events do not significantly contribute to annual SSY (Stock and Tribble, 2009). This approach also assumes that the sediment yield per mm of storm precipitation is constant over the year, and the size distribution of storms has no effect, though there is some evidence that SSY rises exponentially with storm size (Lewis et al, 2001; Rankl, 2004). ")
...
document.add_paragraph("Stream gaging sites were chosen to take advantage of an existing control structure (FG1) and a stabilized stream cross section (FG3). Area-velocity Q measurements could not be made at high stages at FG1 and FG3 for safety reasons, so stage-Q relatiohnships were constructed to estimate a continuous Q record.")
...
document.add_paragraph("Continuous SSC at 15 minute intervals was estimated from 1) 15 min interval turbidity data (T) and a T-SSC relationship calibrated to stream water samples collected over a range of Q and SSC, and 2) linear interpolation of SSC from water samples.")
...
document.add_paragraph("Mean and maximum SSC of water samples, collected during low flow and stormflow by grab and autosampler, were lowest at FG1 (mu="+Mean_SSC_FG1+" mg/L, max="+Max_SSC_FG1+" mg/L), highest at FG2 (mu="+Mean_SSC_FG2+" mg/L, max="+Max_SSC_FG2+"), and in between at FG3 (mu="+Mean_SSC_FG3+" mg/L, max="+Max_SSC_FG3+" mg/L). At FG1, "+Percent_baseflow_FG1+"% of grab samples (n="+No_baseflow_samples('DAM')+") were collected during baseflow conditions (Q_FG1<"+"%.0f"%DAM_Stormflow_conditions+" L/sec), mean SSC: "+Mean_baseflow_SSC('DAM')+" mg/L (Figure 8a); "+Percent_storm_FG1+"% of grab samples (n="+No_storm_samples('DAM')+") were collected during stormflow conditions, mean SSC: "+Mean_storm_SSC('DAM')+" mg/L (Figure 8b). At FG2, "+Percent_baseflow_FG2+"% of grab samples (n="+No_baseflow_samples('DT')+") were collected during baseflow conditions (Q_FG1<"+"%.0f"%DAM_Stormflow_conditions+" L/sec), mean SSC: "+Mean_baseflow_SSC('DT')+" mg/L; "+Percent_stormflow_FG2+"% of grab samples (n="+No_storm_samples('DT')+") were collected during stormflow conditions, mean SSC: "+Mean_stormflow_SSC('DT')+" mg/L. At FG3, "+Percent_baseflow_FG3 +"% of samples (n="+No_baseflow_samples('LBJ')+") were collected during baseflow conditions (Q_FG3<"+"%.0f"%LBJ_Stormflow_conditions+" L/sec), mean SSC: "+Mean_baseflow_SSC('LBJ')+" mg/L; "+Percent_stormflow_FG3+"% of samples (n="+No_storm_samples('LBJ')+") were collected during stormflow conditions, mean SSC: "+Mean_stormflow_SSC('LBJ')+" mg/L. This pattern of SSC values suggests that little sediment is contributed from the forest upstream of FG1, then there is a large input of sediment between FG1 and FG2, and then SSC is diluted by addition of stormflow with lower SSC between FG2 and FG3.")
...
document.add_paragraph("SSYEV from the UPPER and TOTAL watersheds correlated with each of the four storm metrics tested (Figure "+SSY_models_ALL['fig_num']+"). Significant scatter was observed for all models, which reflects the changing sediment availability at the quarry and village, and the natural variability in the watershed response for different storm events. Qmax was the best predictor of SSYEV for both the UPPER and TOTAL watersheds in Faga'alu. The Qmax model for both UPPER and TOTAL watersheds showed the highest coefficient of determination (r2), lowest RMSE, and highest Pearson and Spearman correlation coefficients. Qsum showed an equally high r2, but only for the UPPER subwatershed, and Qsum in both subwatersheds showed higher RMSE than Qmax.")
...
document.add_paragraph("COMBINE THESE PARAGRAGPS Precipitation metrics (Psum and EI30) showed lower correlation coefficients with SSYEV compared to the discharge metrics (Qsum and Qmax) over the full range of storm sizes. SSYEV is calculated from measured Q so it is expected that discharge metrics are more closely correlated, and has been observed in other studies (Rankl, Duvert etc.). EI30 was the least correlated with SSYEV of the storm metrics.                       Discharge metrics showed much higher correlation coefficients than the precipitation metrics in the UPPER subwatershed, but had similar coefficients in the LOWER watershed. This suggests that sediment production processes are more related to discharge processes in the UPPER subwatershed, and more related to precipitation processes in the LOWER subwatershed, influencing the correlation coefficients for the TOTAL watershed. SSY from the LOWER subwatershed is hypothesized to be mostly generated by surface erosion at the quarry, dirt roads, and agricultural plots, whereas SSY from the UPPER subwatershed is hypothesized to be mainly from channel processes and mass wasting. Mass wasting can contribute large pulses of sediment during large precipitation events, which can be deposited near or in the streams and entrained at high discharges during later events. Given the high correlation coefficients for Qmax in both subwatersheds, Qmax may be a promising predictor that integrates both precipitation and discharge processes.")
...
## Storm size
document.add_paragraph("ANCOVA was used to compare regression coefficients of the UPPER and TOTAL SSY models, to determine if the percentage sediment contribution from human-disturbed areas changed with storm size. The model slopes were only significantly different (p<0.05) for the Psum-SSYEV model, but all model intercepts were significantly different (p<0.05). It was hypothesized that for large storms, SSYEV from the UPPER watershed may become relatively more important for total SSY to Faga'alu Bay, however, the models show conflicting results. The Psum-SSYEV models indicate that for larger storm events SSY from the UPPER and TOTAL watersheds are more similar, as the regression lines converge at higher Psum values. Conversely, the Qsum- and Qmax-SSYEV models show no change in relative contributions of SSY over the range of storm sizes (Figure "+SSY_models_ALL['fig_num']+"). In that case, the discharge models (Qsum and Qmax) support the conclusion that human disturbance does not diminish with storm size, while the Psum model supports the conclusion that human-disturbance does diminsh with storm size.")
...
document.add_paragraph("Storm precipitation was "+"{:,g}".format(int(P_2014_storm))+" mm in 2014, representing "+"%.0f"%P_2014_perc_ann+"% of annual total precipitation in 2014 (="+ "{:,g}".format(int(PrecipFilled[start2014:stop2014].sum()))+" mm). ")
...
document.add_paragraph("Several researchers have attempted to explain the difference in alpha (intercept) and Beta (slope) coefficients according to watershed characteristics. A traditional sediment rating curve (Q-SSC) is considered a 'black box' model, and though the alpha and Beta coefficients have no physical meaning, some physical interpretation has been ascribed to them. High alpha values suggest high availability of easily eroded sediment sources in the watershed. High Beta values suggest that small changes in stream discharge lead to large increases in sediment load due to the erosive power of the river or the availability of new sediment sources at high Q (Asselman, 2000). Rankl (2004) found that Beta coefficients were not statistically different between watersheds in semi-arid Wyoming, United States, and he assumed that the Beta exponent was a function of rainfall intensity on hillslopes. Rankl (2004) hypothesized that variability in alpha was a function of sediment availability and erodibility in watersheds. Duvert et al. (2012) argued that alpha values are also dependent on the regression fitting method, arguing that, for instance, nonlinear fitting methods result in a model fit to higher SSY values at lower discharge compared to linear fitting methods.")

document.add_paragraph("In the eight semi-arid watersheds (0.45-22 km2) studied in Duvert et al. (2012) the Alpha coefficients (intercepts in the log-log plots) ranged from 25-5,039 and Beta coefficients (slope in the log-log plots) ranged from 0.95-1.82. Beta coefficients ranged from 1.06-2.45 in eighteen other watersheds from 0.60-1,538km2 in diverse geographical settings (Tropeano, 1991; Basher et al., 1997; Fahey and Marden, 2000; Rankl, 2004; Hicks et al., 2009) compiled by Duvert et al. (2012) . The Alpha coefficients for the Qmax-SSYEV models in Faga'alu were "+alpha_upper+" and "+alpha_total+", and Beta coefficients were " +beta_upper+" and "+beta_total+" in the UPPER and TOTAL Faga'alu watersheds, respectively. The Beta coefficient values are very consistent with the watersheds presented in Duvert 2012, but the alpha coefficient for the undisturbed subwatershed is an order of magnitude lower than the lowest values in Duvert (2012). This suggests that sediment availability is relatively low in Faga'alu, likely due to the dense forest cover over the majority of the watershed.")

document.add_paragraph("In Faga'alu, SSY was least correlated with the Erosivity Index (EI30). Duvert et al. (2012) also found low correlation coefficients with 5 min rainfall intensity for 8 watersheds in France and Mexico. Rodrigues et al. (2013) hypothesized that EI30 is poorly correlated with SSY due to the effect of previous events on antecedent moisture conditions and in-channel sediment storage. Cox (2006) found EI30 was more correlated with soil loss in an agricultural catchment than a forested watershed, and Faga'alu is mainly covered in dense forest. Similar to other studies (Hicks et al., 1990; Fahey et al., 2003; Rankl, 2004; Basher et al., 2011; Duvert et al., 2012; Rodrigues et al., 2013) the highest correlations with SSY were observed for discharge metrics, particularly Qmax which had the highest overall correlation.")
...
document.add_paragraph("Funding for this project was provided by NOAA Coral Reef Conservation Program (CRCP) through the American Samoa Coral Reef Advisory Group (CRAG). Kristine Bucchianeri at CRAG and Susie Holst at NOAA CRCP for providedfunding and support. Christianera Tuitele, Phil Wiles (currently at the South Pacific Regional Environment Programme), and Tim Bodell at American Samoa Environmental Protection Agency (ASEPA), and Fatima Sauafea-Leau and Hideyo Hattori at NOAA, provided support and advice. Dr. Mike Favazza provided critical logistical assistance in American Samoa, and Robert Koch at the American Samoa Coastal Zone Management Program (ASCMP) and Travis Bock at ASEPA assisted in accessing geospatial and water quality data. Many others helped and supported field and laboratory work including Professor Jameson Newtson, Rocco Tinitali, and Valentine Vaeoso at American Samoa Community College (ASCC), Meagan Curtis and Domingo Ochavillo at American Samoa Department of Marine and Wildlife Resources (DMWR), Don and Agnes Vargo at American Samoa Land Grant, Christina Hammock at NOAA American Samoa Climate Observatory, and Greg McCormick at San Diego State University. George Poysky, Jr., George Poysky III, and Mitch Shimasaki at Samoa Maritime Ltd. provided unrestricted access to the Faga'alu quarry site, and historical operation information. Faafetai tele lava.")
