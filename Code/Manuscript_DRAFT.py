...
Percent_QUARRY_S = S_storm_diff_table_quarry()['% LOWER_QUARRY'][-1]
Percent_FOREST_S = S_storm_diff_table_quarry()['% UPPER'][-1]
Percent_VILLAGE_S = S_storm_diff_table_quarry()['% LOWER_VILLAGE'][-1]

document.add_paragraph("SSYEV data measured at FG2 was available for "+"%.0f"%No_of_Storm_Intervals_QUA_S+" of the storms in Table "+S_Diff_table.table_num+", so SSYEV from the LOWER subwatershed including the quarry (SSYLOWER_QUARRY) and the village areas below the quarry (SSYLOWER_VILLAGE) could be calculated to determine the relative sediement contribution from these sources. For the measured storms, Total SSY was comprised of an average of "+Percent_FOREST_S+"% from the UPPER subwatershed, "+Percent_QUARRY_S+"% from the LOWER subwatershed above FG2, and "+Percent_VILLAGE_S+"% from the LOWER subwatershed below FG2. For the storms with valid SSY data at FG1 and FG2 (and not necessarily FG3), specific SSY from the LOWER_QUARRY subwatershed was "+Spec_SSY_Quarry()[2]+"% higher than specific SSY from the UPPER subwatershed ("+Spec_SSY_Quarry()[1]+" and "+Spec_SSY_Quarry()[0]+"). This suggests that the quarry is the most significant sediment source in Faga'alu watershed,  and has significantly increased SSYTOTAL above natural levels.")
