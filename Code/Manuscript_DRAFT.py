...
## SSC Boxplots
## Stormflow
SSC_Boxplots_stormflow = {'filename':figdir+'SSC/Grab sample boxplots stormflow','fig_num':str(fig_count())}
plotSSCboxplots(subset='Pre-storm',withR2=False,show=False,save=True,filename=SSC_Boxplots_stormflow['filename'])
## Baseflow
SSC_Boxplots_baseflow = {'filename':figdir+'SSC/Grab sample boxplots baseflow','fig_num':str(fig_count())}
plotSSCboxplots(subset='Pre-baseflow',withR2=False,show=False,save=True,filename=SSC_Boxplots_baseflow['filename'])
...
plotYSI_compare_ratings(DAM_YSI,DAM_SRC,LBJ_YSI,show_DAM_SRC=True,Use_All_SSC=False,show=False,save=True,filename=LBJ_and_DAM_YSI_Rating_Curve['filename'])
...
document.add_paragraph("SSYEV, m3, km2, SSYUPPER, SSYLOWER, SSYTOTAL, SSYPRE, alpha, Beta, plus-minus")
...
document.add_paragraph("At FG1, recorded stage varied from "+"%.0f"%LBJ['stage'].min()+" to "+"%.0f"%LBJ['stage'].max()+"cm. Manual discharge measurements (n= "+"%.0f"%len(LBJstageDischarge)+") were made from "+"%.0f"%LBJstageDischarge['Q-AV(L/sec)'].min()+" to "+"%.0f"%LBJstageDischarge['Q-AV(L/sec)'].max()+" L/sec, covering a range of stages from "+"%.0f"%LBJstageDischarge['stage(cm)'].min()+" to "+"%.0f"%LBJstageDischarge['stage(cm)'].max()+"cm. The highest recorded stage was more than 2x the highest stage with measured Q so the rating could not be extrapolated by mathematical methods (CITATION about 2x rule of thumb?). The rating at FG1 was extrapolated using Manning's equation and surveyed stream geometry. Manning's n parameter was calibrated using the manual Q measurements (Figure "+LBJ_StageDischarge['fig_num']+"). The RMSE for Mannings modeled Q and Q measurements was "+"%.0f"%LBJ_Man_rmse+" (L/sec), which is used to calculate the total Probable Error. The RMSE for the AV measurements (FG1 and FG3)  from the DUET/WQ look-up table (Harmel et al., 2006) was "+"%.1f"%AV_Q_measurement_RMSE+" %.")
...
document.add_paragraph("At FG1,  "+Percent_storm_FG1+"% of grab samples (n="+No_storm_samples('DAM')+") were taken during stormflow conditions (Q_FG1>"+"%.0f"%DAM_Stormflow_conditions+" L/sec), with mean SSC of "+Mean_storm_SSC('DAM')+" mg/L (Figure "+SSC_Boxplots_stormflow['fig_num']+"), and "+Percent_baseflow_FG1+"% of grab samples (n="+No_baseflow_samples('DAM')+") were taken during baseflow conditions with a mean SSC of "+Mean_baseflow_SSC('DAM')+" mg/L (Figure "+SSC_Boxplots_baseflow['fig_num']+"). At FG2, "+Percent_storm_FG2 +"% of grab samples (n="+No_storm_samples('DT')+") were taken during stormflow conditions (Q_FG1>"+"%.0f"%DAM_Stormflow_conditions+" L/sec), with mean SSC of "+Mean_storm_SSC('DT')+" mg/L, and "+Percent_baseflow_FG2 +"% of grab samples (n="+No_baseflow_samples('DT')+") were taken during baseflow conditions, with mean SSC of "+Mean_baseflow_SSC('DT')+" mg/L. At FG3, "+Percent_storm_FG3+"% of samples (n="+No_storm_samples('LBJ')+") were taken during stormflow conditions (Q_FG3>"+"%.0f"%LBJ_Stormflow_conditions+" L/sec), with mean SSC of "+Mean_storm_SSC('LBJ')+" mg/L, and "+Percent_baseflow_FG3+"% of samples (n="+No_baseflow_samples('LBJ')+") were taken during baseflow conditions, with mean SSC of "+Mean_baseflow_SSC('LBJ')+" mg/L. This pattern of SSC values suggests that little sediment is contributed from the forest upstream of FG1, then there is a large input of sediment between FG1 and FG2, and then SSC is diluted by addition of stormflow with lower SSC between FG2 and FG3.")

## SSC boxplots
# Stormflow
if 'SSC_Boxplots_stormflow' in locals():
    document.add_picture(SSC_Boxplots_stormflow['filename']+'.png',width=Inches(6))
    add_figure_caption(SSC_Boxplots_stormflow['fig_num'],"Boxplots of Suspended Sediment Concentration (SSC) from grab samples only (no Autosampler) at FG1, FG2, and FG3 during storm periods, for (a) the full range of observed values (note: axis is logarithmic) and (b) zoomed in on 0-500 mg/L.")
# Baseflow
if 'SSC_Boxplots_baseflow' in locals():
    document.add_picture(SSC_Boxplots_baseflow['filename']+'.png',width=Inches(6))
    add_figure_caption(SSC_Boxplots_baseflow['fig_num'],"Boxplots of Suspended Sediment Concentration (SSC) from grab samples only (no Autosampler) at FG1, FG2, and FG3 during baseflow periods, for (a) the full range of observed values (note: axis is logarithmic) and (b) zoomed in on 0-500 mg/L.")    
