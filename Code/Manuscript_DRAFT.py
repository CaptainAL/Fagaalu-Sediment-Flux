...
document.add_paragraph("Discharge at both FG1 and FG3 was characterized by periods of low but perennial baseflow (FG1: "+"%.0f"%DAM['Q'].min()+"-"+"%.0f"%DAM_Stormflow_conditions+" L/sec; FG3: "+"%.0f"%LBJ['Q'].min()+"-"+"%.0f"%LBJ_Stormflow_conditions+" L/sec), punctuated by short, flashy hydrograph peaks (FG1: max "+"{:,g}".format(DAM['Q'].max())+" L/sec, FG3: max "+"{:,g}".format(LBJ['Q'].max())+" L/sec) (Figure "+Q_timeseries['fig_num']+").")
...
document.add_paragraph("Mean and maximum SSC of grab and autosamples collected during baseflow and stormflow were lowest at the FG1 site (u="+Mean_SSC_FG1+" mg/L, max="+Max_SSC_FG1+" mg/L), highest at FG2 ("+Mean_SSC_FG2+" mg/L and "+Max_SSC_FG2+") and in between at the downstream FG3 site ("+Mean_SSC_FG3+" mg/L and "+Max_SSC_FG3+" mg/L). At FG1, "+Percent_storm_FG1+"% of grab samples (n="+No_storm_samples('DAM')+") were taken during stormflow conditions (Q_FG1>"+"%.0f"%DAM_Stormflow_conditions+" L/sec), with mean SSC of "+Mean_storm_SSC('DAM')+" mg/L (Figure "+SSC_Boxplots['fig_num']+"), and "+Percent_baseflow_FG1+"% of grab samples (n="+No_baseflow_samples('DAM')+") were taken during baseflow conditions with a mean SSC of "+Mean_baseflow_SSC('DAM')+" mg/L (Figure "+SSC_Boxplots['fig_num']+"(a)). At FG2, "+Percent_storm_FG2 +"% of grab samples (n="+No_storm_samples('DT')+") were taken during stormflow conditions (Q_FG1>"+"%.0f"%DAM_Stormflow_conditions+" L/sec), with mean SSC of "+Mean_storm_SSC('DT')+" mg/L, and "+Percent_baseflow_FG2 +"% of grab samples (n="+No_baseflow_samples('DT')+") were taken during baseflow conditions, with mean SSC of "+Mean_baseflow_SSC('DT')+" mg/L. At FG3, "+Percent_storm_FG3+"% of samples (n="+No_storm_samples('LBJ')+") were taken during stormflow conditions (Q_FG3>"+"%.0f"%LBJ_Stormflow_conditions+" L/sec), with mean SSC of "+Mean_storm_SSC('LBJ')+" mg/L, and "+Percent_baseflow_FG3+"% of samples (n="+No_baseflow_samples('LBJ')+") were taken during baseflow conditions, with mean SSC of "+Mean_baseflow_SSC('LBJ')+" mg/L. This pattern of SSC values suggests that little sediment is contributed from the forest upstream of FG1, then there is a large input of sediment between FG1 and FG2, and then SSC is diluted by addition of stormflow with lower SSC between FG2 and FG3.")
...
document.add_paragraph("SSC varied widely for a given Q at all three sites (FG1, FG2, FG3) due to significant hysteresis observed during storm periods (Figure "+Discharge_Concentration['fig_num']+"). At FG2 and FG3, additional variability in the Q-SSC relationship was due to the changing sediment availability associated with quarrying operations and construction in the village. At FG1, variability of SSC samples during stormflows from year to year was assumed to be caused by randomly occurring landslides caused by large storms.")
...
document.add_paragraph("The highest SSC values were observed just downstream of the quarry (FG2), mainly during discharges that fell below the storm threshold (Figure "+Discharge_Concentration['fig_num']+"). The high SSC during low discharges were due to 1) washing of quarry surfaces during regular maintenance activities at the quarry and 2) rainfall events that did not result in a significant rise in the water level in the stream, but generated sediment-rich runoff from the quarry. Given the close proximity of the quarry to the stream, SSC downstream of the quarry can be highly influenced by mining activity like rock extraction, crushing or hauling operations. During 2012 a common practice for removing fine sediment from crushed aggregate was to rinse it with water pumped from the stream. In the absence of retention structures the fine sediment was then discharged directly into the stream, causing high SSC during interstorm periods. Riverine discharge of fine sediment by rinsing from aggregate was discontinued in 2013, corresponding with a lack of high SSC grab samples during low discharges (Figure "+Discharge_Concentration['fig_num']+"). Currently, waste sediment is piled on-site and severe erosion of these changing stockpiles causes high SSC during storm events.")

document.add_paragraph("The maximum SSC values at FG2 and FG3 were sampled during the same event ("+Max_SSC_FG3_Event+"), during low discharge (Q_FG3="+Max_SSC_FG3_Q+" L/sec)(Figure "+Discharge_Concentration['fig_num']+"). From field observations during this event, a brief but intense rainfall caused high sediment loading from the quarry, but did not increase Q above the defined storm threshold. The low amount of Q from forested areas draining to FG2 was too low to dilute the high amount of sediment from the quarry and village, causing the extremely high SSC. SSC was diluted further downstream at FG3 by the addition of runoff from impervious surfaces in the village. The maximum SSC value for the upstream site was sampled on "+Max_SSC_FG1_Event+" at high discharge (Q_FG1= "+Max_SSC_FG1_Q+" L/sec). Anecdotal and field observations reported higher than normal SSC upstream of the quarry during the 2013 field season, possibly due to landsliding from large storms (G. Poysky, pers. comm.).") 

#### Comparing SSY from disturbed and undisturbed subwatersheds
document.add_heading('Comparing SSY from disturbed and undisturbed subwatersheds',level=3)
document.add_paragraph("A main objective for this study was to determine how much human disturbance has increased Total SSY to Faga'alu Bay (SSYTOTAL). SSYUPPER was measured at FG1, SSYTOTAL was measured at FG3, and SSYLOWER was calculated by subtracting SSYUPPER from SSYTOTAL (Table "+S_Diff_table.table_num+").")
