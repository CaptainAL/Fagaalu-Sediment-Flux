...
document.add_paragraph("SSY generated by individual storm events (SSYEV) can be used to assess the contribution of individual subwatersheds to total SSY (Zimmermann et al., 2012), compare the responses of different watersheds to "+'"storm metrics"'+" (Basher et al., 2011; Duvert et al., 2012; Fahey et al., 2003; Hicks, 1990),  and determine changes in SSY from the same watershed over time (Bonta, 2000). SSYEV is calculated by integrating continuous suspended sediment yield from measured or modeled water discharge (Q) and measured or modeled suspended sediment concentration (SSC) (Duvert et al., 2012):")
...
document.add_paragraph("To assess the contribution of each subwatershed to total SSY from Faga'alu watershed, the percent contributions were calculated from the Upper, undisturbed subwatershed (SSYUPPER=SSYFOREST), and the Lower, human-disturbed subwatershed (SSYLOWER) for storm events with available data at both gaging locations (FOREST and VILLAGE-Figure "+Study_Area_map['fig_num']+"). SSYLOWER, which includes the quarry and  village areas, was calculated by subtracting SSY measured at FOREST from SSY measured at VILLAGE (SSYLOWER = SSYVILLAGE - SSYFOREST).")
document.add_paragraph("Another approach to quantifying how much SSY has increased from anthropogenic disturbance is the disturbance ratio (DR). The DR is the ratio of SSYEV from the total human-disturbed watershed under current conditions (measured at the watershed outlet (VILLAGE)) to SSY under pre-disturbance conditions ( SSYPRE):")
...
document.add_paragraph("From January 6, 2012, to October 1, 2014, "+str(len(SSC_dict['Pre-ALL']))+" samples were collected at 12 sites in Faga'alu and analyzed for SSC. Three sites were the focus of this analysis: 1) FOREST(n="+"%.0f"%No_All_samples('DAM')+"), 2) QUARRY (n="+"%.0f"%No_All_samples('DT')+" grab samples, n="+"%.0f"%No_All_samples('R2')+" from the Autosampler), and 3) VILLAGE (n="+"%.0f"%No_All_samples('LBJ')+"). Mean SSC of grab samples collected during baseflow and stormflow were lowest at the FOREST site ("+Mean_SSC_Forest+" mg/L), and highest at the QUARRY ("+Mean_SSC_Quarry+" mg/L) and the downstream VILLAGE site ("+Mean_SSC_Village+" mg/L). Maximum SSC values were "+Max_SSC_Forest+" mg/L, "+Max_SSC_Quarry+" mg/L, and "+Max_SSC_Village+" mg/L at the upstream (FOREST), quarry (QUARRY), and downstream (VILLAGE) sites, respectively. The maximum SSC values at QUARRY ("+Max_SSC_Quarry+" mg/L ) and VILLAGE ("+Max_SSC_Village+" mg/L) were sampled during the same event ("+Max_SSC_Village_Event+"), during fairly low discharge (Q_VILLAGE="+Max_SSC_Village_Q+" L/sec). The maximum SSC value for the upstream site ("+Max_SSC_Forest+" mg/L) was sampled on "+Max_SSC_Forest_Event+" at high discharge (Q_FOREST= "+Max_SSC_Forest_Q+" L/sec). Anecdotal and field observations reported higher than normal SSC upstream of the quarry during the 2013 field season, possibly due to landsliding from large storms (G. Poysky, pers. comm.).")
...
document.add_paragraph("Most storm events showed a typical pattern, where a short period of intense rainfall caused a rapid increase in SSC downstream of the quarry (QUARRY) while SSC remained low at FOREST, indicating rapid sheetwash of sediment from the quarry into the stream (Figure "+Example_Storm['fig_num']+"). Highest event SSC was typically observed at QUARRY, with slightly lower and later peak SSC observed at VILLAGE. SSC at FOREST typically increased slower, remained much lower, and peaked later than the downstream sites (QUARRY and VILLAGE). Though peak SSC was highest at QUARRY, the total SSY was highest at VILLAGE due to the addition of storm runoff from the relatively larger watershed downstream. Storm flow at VILLAGE includes both storm runoff from disturbed areas of the quarry and village, and the undisturbed forest on the steep hillsides in the lower watershed . Complex storm events occurred when a subsequent period of rainfall followed before Q and SSC fell back to baseflow levels. Several complex events where Q peaks were separated by a reasonable period of time and Q was nearly at baseflow were separated into individual storm events.")
...
document.add_paragraph("SSYEV data measured at QUARRY was available for "+"%.0f"%No_of_Storm_Intervals_QUA_S+" of the storms in Table "+S_Diff_table.table_num+", so SSYEV from the quarry and village areas could be calculated to determine the relative sediement contribution from these sources. For the measured storms, Total SSY was comprised of an average of "+Percent_FOREST_S+"% from the Upper subwatershed, "+Percent_QUARRY_S+"% from the Quarry subwatershed, and "+Percent_VILLAGE_S+"% from the Lower subwatershed (downstream of the Quarry). For the storms with valid SSY data at FOREST and QUARRY (and not necessarily VILLAGE), specific SSY from the Quarry subwatershed was "+Spec_SSY_Quarry()[2]+"% higher than specific SSY from the Upper subwatershed ("+Spec_SSY_Quarry()[1]+" and "+Spec_SSY_Quarry()[0]+"). This suggests that the quarry is the most significant sediment source in Faga'alu watershed,  and has significantly increased Total SSY above natural levels.")
