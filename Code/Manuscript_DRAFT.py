...
document.add_paragraph("SSYEV, m3, km2, SSYUPPER, sSSYUPPER, SSYLOWER, sSSYLOWER, SSYTOTAL, sSSYUPPER, alpha, SSYFG3, SSYFG2, SSYFG1, SSYLOWER_QUARRY,SSYLOWER_VILLAGE, Beta, plus-minus, r2")
...
document.add_paragraph("The disturbance ratio (DR) is the ratio of SSYEV from the total human-disturbed watershed under current conditions (SSYTOTAL), to SSY under pre-disturbance conditions, calculated by the specific SSY from the UPPER subwatershed:")
add_equation(DR_eq) ## Equation
document.add_paragraph("It is assumed that the whole watershed was originally covered in forest, with sSSY from forested areas in the LOWER subwatershed being equal to sSSY from the undisturbed UPPER subwatershed. SSY estimated for the disturbed portions of the LOWER subwatershed (Equation "+SSY_disturbed.eq_num+") was used to calculate a DR for the disturbed areas in the LOWER subwatershed.").paragraph_format.first_line_indent = Inches(0.0)
...
document.add_paragraph("To determine if sediment contributions from human-disturbed areas and undisturbed areas varied with storm size (Lewis et al., 2001), an Analysis of Covariance (ANCOVA) was used to determine if  the regression slopes were statistically different (Rankl, 2004). If regression slopes for the UPPER and TOTAL watersheds are significantly different, it supports the conclusion that the effect of human-disturbance changes with storm size.")
...
document.add_paragraph("Annual SSY and sSSY were also estimated by extrapolating SSY from measured storms by the ratio of annual storm precipitation to the precipitation measured during sampled storms:")
...
document.add_paragraph("Continuous SSC at 15 min intervals to calculate SSYEV (Equation "+SSYEV_eq.eq_num+") was estimated from 1) 15 min interval turbidity data (T) and a T-SSC relationship calibrated to stream water samples collected over a range of Q and SSC, and 2) linear interpolation of SSC from grab and autosamples.")
...
document.add_paragraph("Duvert et al. (2012) compiled Qmax-SSY results from twenty-eight watersheds (0.45-1,538km2) and found Beta coefficients (slope in the log-log plots) that ranged from 1.06-2.45, and alpha coefficients (intercepts in the log-log plots) that ranged from 25-5,039. The alpha coefficients for the Qmax-SSYEV models in Faga'alu were "+alpha_upper+" and "+alpha_total+", and Beta coefficients were " +beta_upper+" and "+beta_total+" in the UPPER and TOTAL Faga'alu watersheds, respectively. The Beta coefficient values are very consistent with the watersheds presented in Duvert 2012, but the alpha coefficient for the undisturbed subwatershed is an order of magnitude lower than the lowest values in Duvert (2012). This suggests that sediment availability is relatively low in Faga'alu, likely due to the dense forest cover over the majority of the watershed.")

document.add_paragraph("In Faga'alu, SSY was least correlated with the Erosivity Index (EI30). Duvert et al. (2012) also found low correlation coefficients with 5 min rainfall intensity for 8 watersheds in France and Mexico. Rodrigues et al. (2013) hypothesized that EI30 is poorly correlated with SSY due to the effect of previous events on antecedent moisture conditions and in-channel sediment storage. Cox (2006) found EI30 was more correlated with soil loss in an agricultural catchment than a forested watershed, and Faga'alu is mainly covered in dense forest. Similar to other studies (Hicks et al., 1990; Fahey et al., 2003; Rankl, 2004; Basher et al., 2011; Duvert et al., 2012; Rodrigues et al., 2013) the highest correlations were observed for discharge metrics, particularly Qmax which had the highest overall correlation with a Spearman correlation coefficient of "+"%.2f"%QmaxS_upper.spearman+" for the UPPER subwatershed and "+"%.2f"%QmaxS_total.spearman+" for the TOTAL watershed.")
...
document.add_paragraph("Funding for this project was provided by NOAA Coral Reef Conservation Program (CRCP) through the American Samoa Coral Reef Advisory Group (CRAG). We would like to acknowledge Kristine Bucchianeri at CRAG and Susie Holst at NOAA CRCP for providing funding and support for this project. We would like to thank Christianera Tuitele and Tim Bodell at American Samoa Environmental Protection Agency (ASEPA), and Fatima Sauafea-Leau and Hideyo Hattori at NOAA, for their support and advice. We would also like to thank Dr. Mike Favazza for providing critical logistical assistance in American Samoa, and Robert Koch at the American Samoa Coastal Zone Management Program (ASCMP) and Travis Bock at ASEPA for their assistance in accessing geospatial and water quality data. We would like to thank all those who helped and supported field and laboratory work including Professor Jameson Newtson, Rocco Tinitali, and Valentine Vaeoso at American Samoa Community College (ASCC), Meagan Curtis and Domingo Ochavillo at American Samoa Department of Marine and Wildlife Resources (DMWR), Don and Agnes Vargo at American Samoa Land Grant, Christina Hammock at NOAA American Samoa Climate Observatory, and Greg McCormick at San Diego State University. We would also like to acknowledge George Poysky, Sr., George Poysky III, and Mitch Shimasaki at Samoa Maritime Ltd. for providing unrestricted access to the Faga'alu quarry site, and historical operation information. Faafetai tele lava.")
