...
def No_All_samples(location):
    ...
    return No_samples
document.add_paragraph("Stream water samples for SSC were collected by dip sampling with 500mL HDPE bottles at FOREST, QUARRY, and VILLAGE, and by an ISCO 3700 Autosampler at QUARRY. Other sites were sampled opportunistically during storm events. From January 6, 2012, to October 1, 2014, "+str(len(SSC_dict['Pre-ALL']))+" samples were collected at 12 sites in Faga'alu and analyzed for SSC. Samples were analyzed in the field using gravimetric methods (Gray et al., 2000). Water samples were vacuum filtered on 47mm, 0.7um Millipore AP40 glass fiber filters, oven dried at 100C for one hour, cooled and weighed to determine SSC (mg/L). Three main sampling locations in Faga'alu are the focus in this analysis: 1) Upstream (FOREST)(n="+"%.0f"%No_All_samples('DAM')+"), 2) immediately downstream of the Quarry (QUARRY)(n="+"%.0f"%No_All_samples('DT')+" from grab sampling, n="+"%.0f"%No_All_samples('R2')+"), and 3) Downstream (VILLAGE)(n="+"%.0f"%No_All_samples('LBJ')+").")
...
document.add_paragraph("Turbidity (T) was measured at FOREST and VILLAGE using three types of turbidimeters: a Greenspan TS3000, a YSI 600OMS, and a CampbellSci OBS500. All turbidimeters were permanently installed in protective PVC housings where the turbidity probe would be continuously submerged and oriented downstream. Despite regular monitoring, debris fouling during storm and baseflows was common and caused data loss during several storm events. The YSI turbidimeter was recalibrated with YSI NTU standards at the beginning of each successive field season, approximately every 3-6 months during data collection. All turbidimeters were regularly cleaned following storms to ensure proper operation.")
...
Max_SSC_Village_Event = str(SSC_dict['Pre-ALL'][SSC_dict['Pre-ALL']['Location'].isin(['LBJ'])]['SSC (mg/L)'].idxmax())
Max_SSC_Village_Q = LBJ['Q'][SSC_dict['Pre-ALL'][SSC_dict['Pre-ALL']['Location'].isin(['LBJ'])]['SSC (mg/L)'].idxmax()]
Max_SSC_Forest_Event = str(SSC_dict['Pre-ALL'][SSC_dict['Pre-ALL']['Location'].isin(['DAM'])]['SSC (mg/L)'].idxmax())
...
def Percent_storm(location):
    ...
    percent = (storm_samples/all_samples) * 100.0
    return percent
    
document.add_paragraph("At FOREST, "+"%.0f"%No_storm_samples('DAM')+" grab samples ("+"%.0f"%Percent_storm('DAM')+"%) were taken during stormflow conditions (Q_DAM>"+"%.0f"%DAM_Stormflow_conditions+" L/sec), with mean SSC of "+"%.0f"%Mean_storm_SSC('DAM')+" mg/L. At QUARRY, "+"%.0f"%No_storm_samples('DT')+" grab samples ("+"%.0f"%Percent_storm('DT')+"%) were taken during stormflow conditions (Q_DAM>"+"%.0f"%DAM_Stormflow_conditions+" L/sec); and mean SSC for QUARRY stormflow samples was "+"%.0f"%Mean_storm_SSC('DT')+" mg/L. At VILLAGE, "+"%.0f"%No_storm_samples('LBJ')+" samples ("+"%.0f"%Percent_storm('LBJ')+"%) were taken during stormflow conditions (Q_VILLAGE>"+"%.0f"%LBJ_Stormflow_conditions+" L/sec); and mean SSC for VILLAGE stormflow samples was "+"%.0f"%Mean_storm_SSC('LBJ')+" mg/L (Figure "+SSC_Boxplots['fig_num']+". This pattern of SSC values suggests there is a large input of sediment downstream of FOREST at the quarry, and then SSC is diluted by addition of stormflow with lower SSC between QUARRY and VILLAGE.")

document.add_paragraph("Given the close proximity of the quarry to the stream, SSC downstream of the quarry can be highly influenced by mining activity. Prior to 2013, a common practice for removing fine sediment from crushed aggregate was to rinse it with water pumped from the stream. The fine sediment was discharged directly into the stream, causing high SSC during low Q. While sheetwash erosion of the quarry during storms causes higher total sediment loading, the instantaneous SSC values are lower due to dilution by stormflow. The practice of manually rinsing fine sediment from aggregate was discontinued in 2013, corresponding with a lack of high SSC grab samples during low discharges (Figure "+Discharge_Concentration['fig_num']+").")
