...
document.add_paragraph("Stream water samples for SSC were collected by dip sampling with 500mL HDPE bottles at FOREST, QUARRY, and VILLAGE, and by an ISCO 3700 Autosampler at QUARRY. Other sites were sampled opportunistically during storm events. From January 6, 2012, to October 1, 2014, "+str(len(SSC_dict['Pre-ALL']))+" samples were collected at 12 sites in Faga'alu and analyzed for SSC. Samples were analyzed in the field using gravimetric methods (Gray et al., 2000). Water samples were vacuum filtered on 47mm, 0.7um Millipore AP40 glass fiber filters, oven dried at 100C for one hour, cooled and weighed to determine SSC (mg/L). Three main sampling locations in Faga'alu are the focus in this analysis: 1) Upstream (FOREST)(n="+str(DAM_grab_count)+"), 2) immediately downstream of the Quarry (QUARRY)(n="+str(QUARRY_grab_count)+" from grab sampling, n="+str(QUARRY_R2_grab_count)+"), and 3) Downstream (VILLAGE)(n="+str(LBJ_grab_count)+").")
...
document.add_paragraph("Discharge at both FOREST and VILLAGE was characterized by periods of low but perennial baseflow (FOREST: "+"%.0f"%DAM['Q'].min()+"-"+"%.0f"%DAM[DAM['stage']==DAM_storm_threshold.round(0)]['Q'][0]+" L/sec; VILLAGE: "+"%.0f"%LBJ['Q'].min()+"-"+"%.0f"%LBJ[LBJ['stage']==LBJ_storm_threshold.round(0)]['Q'][0]+" L/sec), punctuated by short, flashy hydrograph peaks (FOREST: max "+"%.0f"%DAM['Q'].max()+" L/sec, VILLAGE: max "+"%.0f"%LBJ['Q'].max()+" L/sec). ")
