...
document.add_paragraph("Annual estimates of SSY and sSSY were used to compare Faga'alu with other literature. A continuous annual time-series of SSY was not possible at the study site due to the discontinuous field campaigns and failure of or damage to the turbidimeters during some months. Continuous records of P and Q were available for 2014, so the Psum-SSYEV and Qmax-SSYEV models (Eq 4) were used to predict SSY for all storms in 2014 (Basher et al., 1997). Sediment mitigation structures were installed at the quarry in October 2014, greatly reducing SSY from the LOWER_QUARRY subwatershed (unpublished data), so the Qmax-SSY relationship developed prior to the mitigation was used to calculate the annual pre-mitigation sediment yield. For storms with no Qmax data at FG3, Qmax was predicted from a linear regression between Qmax at FG1 and Qmax at FG3 for the study period (not shown).")
