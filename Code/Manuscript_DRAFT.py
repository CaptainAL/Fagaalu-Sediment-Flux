...
plot_storm_individually(LBJ_StormIntervals.loc[121],show=False,save=True,filename=Example_Storm['filename']) 
...
document.add_paragraph("The study watershed, Faga'alu, is located on Tutuila (14S, 170W), the largest island in the Territory of American Samoa (140 km2). Like many volcanic islands in the Pacific, Tutuila is composed of steep, heavily forested mountains with villages and roads confined to the flat areas near the coast. The main stream in Faga'alu runs the length of the watershed (~3 km), and drains an area of 1.86 km2 (area draining to FG3 in Figure 1). The main watershed includes Matafao Mountain, the highest point on Tutuila (653 m), and the stream discharges into the Pacific Ocean. The mean slope of the main Faga'alu watershed is 0.53 m/m and total relief is 653 m. Several small ephemeral streams drain directly to the ocean (0.63 km2) (grey dotted boundary in Figure 1  ).  The stream discharges to an adjacent, fringing coral reef embayment that Fenner et al. (2008) identified as being highly degraded by sediment. Faga'alu watershed was identified by local environmental management agencies in the American Samoa Coral Reef Advisory Group (CRAG) as a heavily impacted watershed, and in August 2012 was selected by the US Coral Reef Task Force (USCRTF) as a Priority Watershed for conservation and remediation efforts (Holst-Rice et al., 2015).")
...
document.add_paragraph("The predominant land cover in Faga'alu watershed   is undisturbed vegetation (93.2%), including forest (84.5%) and scrub/shrub (8.6%) on the steep hillsides (Table 1), based on a land cover map from NOAA’s Ocean Service and Coastal Services Center (2010). The upper watershed is dominated by undisturbed rainforest on steep hillslopes. The lower subwatershed has steep vegetated hillslopes and a relatively small flat area in the valley bottom that is urbanized. This settlement pattern is typical in the South Pacific and other volcanic islands, where their small size and steep topography constrain development to valley bottoms near the coast (Bégin et al., 2014).  Compared to other watersheds on Tutuila, a relatively large portion of Faga'alu watershed is urbanized (4.6% "High Intensity Developed" in Table 1), due to large areas of impervious surface associated with the hospital and the numerous residences and businesses. A small portion of the watershed (1.1%) is developed open space, which includes landscaped lawns and parks. In addition to some small household gardens there are several small agricultural areas of banana and taro on the steep hillsides. These agricultural plots were classified as grassland due to the high fractional grass cover in the plots. Farmers of these plots receive technical assistance from the Natural Resource Conservation Service (NRCS) to mitigate erosion. There are several small footpaths and unpaved driveways in the village, but most unpaved roads are stabilized with compacted gravel and do not appear to be a major contributor of sediment (Horsley-Witten, 2012). Longitudinal sampling of Faga'alu stream during low flow conditions in 2011 showed significantly increased turbidity downstream of a bridge construction site on the village road approximately 200 m downstream of FG2 (Curtis et al., 2011). Construction of the bridge was completed in March 2012 and no longer increases turbidity.")
...
document.add_paragraph("The suspended sediment yield (SSY) in Faga'alu stream was measured at three sampling points that drain key land covers the we hypothesized would have different SSY: FG1 drains undisturbed forest in the UPPER subwatershed (watershed boundary to FG1), FG2 drains undisturbed forest and the quarry in the LOWER_QUARRY subwatershed (between FG1 and FG2), and FG3 drains undisturbed forest and the village in the LOWER_VILLAGE subwatershed (between FG2 and FG3) (Table 1). FG3 is also the watershed outlet for the TOTAL watershed.")
...
###### REWRITE SECTION
document.add_paragraph("Storm events can be defined by precipitation (Hicks, 1990) or discharge parameters (Duvert et al., 2012), and the method used to identify storm events on the hydrograph can significantly influence the analysis of SSYEV (Gellis, 2013). Complex graphical or rule-based techniques for hydrograph separation may be implemented (Dunne and Leopold, 1978; Perreault, 2010), but for this research the simple stage height threshold rule was used due to the flashy hydrologic response, low baseflow discharge, and short duration of recession curves between events (Fahey et al., 2003; Lewis et al., 2001). A storm event was defined as the period of time when stream stage height exceeded a given threshold for a minimum of two hours. The storm threshold was defined as the mean stage recorded over the study period, plus one standard deviation and the term "baseflow" is used to designate periods when flow was below the storm threshold or exceeded the threshold for less than two hours. Complex storm events occurred when additional rain fell before the stream stage fell below the storm threshold. Events with multiple Q peaks were separated into separate events if the peaks were separated by at least a two hour period and Q was near the storm threshold. Several small events produced sediment runoff and high SSC but did not meet the storm definition criteria to be included as a storm event.")
...
document.add_paragraph("The regression coefficients (alpha and Beta) for the UPPER and TOTAL watersheds were tested for statistically significant differences using Analysis of Covariance (ANCOVA) (Lewis et al., 2001). A higher intercept (alpha) for the human-disturbed watershed indicates higher sediment yield for the same size storm event, compared to sediment yield from undisturbed areas. A difference in slope (Beta) would indicate the relative sediment contributions from the subwatersheds change with increasing storm size. If regression slopes for the UPPER and TOTAL watersheds are significantly different, it supports the conclusion that the effect of human-disturbance changes with storm size.")
...
document.add_paragraph("At FG1, the flow control structure is a masonry ogee spillway crest of a defunct stream capture. The structure is a rectangular channel 43 cm deep that transitions abruptly to gently sloping banks, causing an abrupt change in the stage-Q relationship (Appendix Figure "+DAM_Cross_Section['fig_num']+"). At FG1, recorded stage height ranged from "+"%.0f"%DAM['stage'].min()+" to "+"%.0f"%DAM['stage'].max()+" cm, while area-velocity Q measurements (n= "+"%.0f"%len(DAMstageDischarge)+") covered stages from "+"%.0f"%DAMstageDischarge['stage(cm)'].min()+" to "+"%.0f"%DAMstageDischarge['stage(cm)'].max()+" cm. Since the highest recorded stage ("+"%.0f"%DAM['stage'].max()+" cm) was higher than the highest stage with measured Q ("+"%.0f"%DAMstageDischarge['stage(cm)'].max()+" cm), and there was a distinct change in channel geometry above 43 cm the rating could not be extrapolated by a power law. The flow structure did not meet the assumptions for using Manning's equation to predict flow so the HEC-RAS model was used (Brunner 2010). The surveyed geometry of the upstream channel and flow structure at FG1 were input to HEC-RAS, and the HEC-RAS model was calibrated to the Q measurements (Figure "+DAM_StageDischarge['fig_num']+"). While a power function fit Q measurements better than HEC-RAS for low flow, HEC-RAS fit better for Q above the storm threshold used in analyses of SSY (Figure "+DAM_StageDischarge['fig_num']+").")
...
document.add_paragraph("Interpolation of SSC values from grab samples could only be performed if more than three stream water samples were collected during a storm event, and if they adequately captured the SSC dynamics of the storm event. SSC was assumed to be zero at the beginning and end of each storm if no grab sample data was available for those times (Lewis et al., 2001).")
...
document.add_paragraph("SSYEV was measured simultaneously at FG1 and FG3 for "+S_budget['Storm Start']['Total/Avg']+" storms (Table "+S_budget_table.table_num+"). SSYTOTAL was "+SSY_TOTAL_2+u"\u00B1"+"%.1f"%PE_SSY_TOTAL_2+" tons ("+sSSY_TOTAL_2+u"\u00B1"+"%.1f"%PE_sSSY_TOTAL_2+" tons/km2), with "+SSY_UPPER_2+u"\u00B1"+"%.1f"%PE_SSY_UPPER_2+" tons ("+sSSY_UPPER_2+u"\u00B1"+"%.1f"%PE_sSSY_UPPER_2+" tons/km2) from the UPPER subwatershed and "+SSY_LOWER_2+" tons ("+sSSY_LOWER_2+" tons/km2) from the LOWER subwatershed. The UPPER and LOWER subwatersheds are similar in size (0.90 km2  and 0.88 km2 ) but SSYUPPER accounted for an average of just "+Percent_Upper_S_2 +"% and SSYLOWER for "+Percent_Lower_S_2 +"% of SSY TOTAL at the watershed outlet (Table "+S_budget_table.table_num+"). The DR estimated from sSSYUPPER and sSSYLOWER suggests sSSY has increased by "+S_budget['LOWER tons']['DR']+"x in the LOWER subwatershed, and "+S_budget['TOTAL tons']['DR']+"x for the TOTAL watershed.")


document.add_paragraph("The measured sSSY from the forested UPPER watershed (sSSYUPPER="+sSSY_UPPER_2+" tons/km2) was used to calculate SSY from undisturbed forest in the LOWER subwatershed. SSY from the undisturbed forest areas in the LOWER watershed was "+S_budget_analysis_table['LOWER']['Forested areas']+" tons, so SSY from the disturbed areas was "+S_budget_analysis_table['LOWER']['Disturbed areas']+" tons (Equation "+SSY_disturbed.eq_num+"). For the storms in Table "+S_budget_table.table_num+", roughly "+S_budget_analysis['LOWER']['percent from disturbed areas']+"% of SSYLOWER was from disturbed areas, despite the disturbed areas only accounting for "+S_budget_analysis['LOWER']['Fraction of subwatershed area disturbed']+"% of the LOWER subwatershed area (0.089 km2). Similarly, despite only "+S_budget_analysis['TOTAL']['Fraction of subwatershed area disturbed']+"% of the TOTAL watershed being disturbed, SSY from disturbed areas accounted for "+S_budget_analysis['TOTAL']['percent from disturbed areas']+"% of the SSYTOTAL. sSSY from disturbed areas in the LOWER subwatershed was "+"{:,g}".format(float(S_budget_analysis['LOWER']['sSSY, disturbed areas (tons/km2)']))+" tons/km2, or "+S_budget_analysis['LOWER']['DR for sSSY from disturbed areas']+"x the sSSY of undisturbed forest.")
...
document.add_paragraph("The Qmax-SSY relationships were used to predict SSY from Qmax of "+str(len(no_storms_2014))+" storms in 2014, the only year with a continuous Q record (Table "+est_Annual_SSY_table.table_num+"). Predicted annual SSY in 2014 from the UPPER and TOTAL watersheds was "+SSY_Upper_Qmax_2014+" and "+SSY_Total_Qmax_filled_2014+" tons/year, respectively. Predicted annual sSSY in 2014 from the UPPER and TOTAL watersheds, was "+sSSY_Upper_Qmax_2014+" and "+sSSY_Total_Qmax_filled_2014+" tons/km2/year, respectively.")

## Annual SSY and sSSY from full range of measured storms at FG1 and FG3
document.add_paragraph("Annual SSY was also calculated using Equation "+SSY_annual_eq.eq_num+" for three sets of storm events: a) all events with SSYEV data, including those where SSYEV data were only available for a single site; b) only events where data was available for both UPPER (FG1) and TOTAL (FG3) and c) only events where data was available for UPPER (FG1), LOWER_QUARRY (FG2), and TOTAL (FG3). Including all storms (method a) will provide the best estimate at a given location, while b) and c) allow more direct comparison of different subwatersheds. Continuous records of Q and precipitation in 2014 showed annual storm precipitation (Psann) was "+"{:,g}".format(int(P_2014_storm))+" mm, representing "+P_2014_perc_ann+"% of total annual precipitation ("+ "{:,g}".format(int(PrecipFilled[start2014:stop2014].sum()))+" mm). All storms with measured SSY at FG1 from 2012-2014 included"+ "{:,g}".format(int(P_FG1_all_storms))+" mm of precipitation (Psmeas), or "+"%.0f"%P_FG1_percent_storm+"% of Psann, so estimated annual SSY from the UPPER subwatershed from Equation "+SSY_annual_eq.eq_num+" was "+"%.0f"%annual_SSY_UPPER_ALL+" tons/yr ("+"%.0f"%annual_sSSY_UPPER_ALL+" tons/km2 /yr). All storms with measured SSY at FG3 from 2012-2014 included "+"{:,g}".format(int(P_FG3_all_storms))+" mm of precipitation, or "+"%.0f"%P_FG3_percent_storm+"% of expected annual storm precipitation so estimated annual SSY from the TOTAL watershed was "+"%.0f"%annual_SSY_TOTAL_ALL+" tons/yr ("+"%.0f"%annual_sSSY_TOTAL_ALL+" tons/km2 /yr).")
    

## Annual SSY and sSSY from Table 2 (UPPER and LOWER)
document.add_paragraph("For storms with measured SSY at both FG1 and FG3 (Table "+S_budget_2_table.table_num+") Psmeas was "+"{:,g}".format(int(P_measured_2))+" mm, or "+"%.0f"%P_measured_2_perc_storm+"% of Psann. Using Equation "+SSY_annual_eq.eq_num+", estimated annual SSY from the UPPER, LOWER, and TOTAL watersheds was "+annual_SSY_UPPER_2+", "+annual_SSY_LOWER_2+", and "+annual_SSY_TOTAL_2+" tons/year, respectively. Estimated annual sSSY from the UPPER, LOWER, and TOTAL watersheds was "+annual_sSSY_UPPER_2+", "+annual_sSSY_LOWER_2+", and "+annual_sSSY_TOTAL_2+" tons/km2/year, respectively.")


## Annual SSY and sSSY from Table 4 (UPPER, LOWER_QUARRY, LOWER_VILLAGE, LOWER)
document.add_paragraph("For storms with measured SSY at FG1, FG2, and FG3 (Table "+S_budget_2_table.table_num+") Psmeas was "+P_measured_4+" mm, or "+"%.0f"%P_measured_4_perc_storm+"% of Psann. Using Equation "+SSY_annual_eq.eq_num+", estimated annual SSY from the UPPER, LOWER_QUARRY, LOWER_VILLAGE, LOWER, and TOTAL watersheds was "+annual_SSY_UPPER_3+", "+annual_SSY_LOWER_QUARRY_4+", "+annual_SSY_LOWER_VILLAGE_4+", "+annual_SSY_LOWER_4+", and "+annual_SSY_TOTAL_4+" tons/year, respectively. Annual sSSY from the UPPER, LOWER_QUARRY, LOWER_VILLAGE, LOWER, and TOTAL watersheds were estimated to be "+annual_sSSY_UPPER_4+", "+annual_sSSY_LOWER_QUARRY_4+", "+annual_sSSY_LOWER_VILLAGE_4+", "+annual_sSSY_LOWER_4+" and "+annual_sSSY_TOTAL_4+" tons/km2/year, respectively.")
...
document.add_paragraph("Milliman and Syvitski (1992) report there is an unusually high average sSSY of 1,000-3,000 tons/km2 /year, from watersheds (10-100,000 km2) in tropical Asia and Oceania. However, Milliman and Syvitski's (1992) regional models of sSSY as a function of basin size and maximum elevation predict only 13 tons/km2 /year from watersheds with peak elevation 500-1,000 m (highest point of UPPER Faga'alu subwatershed is 653 m), but 68 tons/km2 /year for max elevations of 1,000-3,000 m,   which is comparable to SSY measured from UPPER Faga’alu watershed (33-80 tons/km2 /yr) (Table 8). Given the high vegetation cover and lack of human activity in the UPPER Faga'alu subwatershed, it is assumed that sSSY should be several orders of magnitude lower than watersheds presented in Milliman and Syvitski (1992)   but sSSY from the forested UPPER Faga'alu subwatershed was approximately two times higher. However, the UPPER subwatershed is a smaller watershed than included in Milliman and Syvitski (1992) (smallest 100 km2), with less sediment storage, and orders of magnitude scatter around their model is observed for smaller watersheds (Figures 5e and 6e in Milliman and Syvitski 1992).")

# compare to Hawaii watersheds
document.add_paragraph("Few examples of sediment yield studies on volcanic, Pacific Islands similar to Tutuila were found in the literature for comparison, except for studies in two Hawaiian watersheds (Stock and Tribble, 2010): Hanalei watershed on Kauai, and Kawela watershed on Molokai (Table "+lit_values_for_Annual_ssy_table_num+"). Where mean and maximum SSC values were similar to Faga'alu, sSSY was also similar. Hanalei watershed on Kauai (54 km2), has similarly steep relief and high rainfall (varies with elevation from 2,000-11,000 mm), with average SSC of 63 mg/L and maximum SSC of 2,750 mg/l. Calhoun and Fletcher (1999) previously estimated sSSY from Hanalei was 140±55 tons/km2/year, but had fewer data than Stock and Tribble (2010), who estimated sSSY was 525 tons/km2 /yr. In Kawela watershed on Molokai (14 km2), a grazing-disturbed sub-humid watershed (precipitation varies with elevation from 500-3,000 mm), Stock and Tribble (2010) estimated sSSY was 459 tons/km2/yr. In Kawela, Molokai, SSC was much higher than measured in Faga'alu and occurred at relatively lower flow, with average SSC of 3,490 mg/L, and a maximum value of 54,000 mg/L at an instantaneous flow of 1.614 m3 /sec. In the disturbed, TOTAL Faga'alu watershed where average SSC was  "+Mean_SSC_FG3+" mg/L and maximum SSC was "+Max_SSC_FG3+" mg/L, annual sSSY was estimated to be 214-250 tons/km2/yr, similar to Hanalei watershed.")


# compare specific land covers
document.add_paragraph("Annual sSSY from the disturbed quarry was estimated to be approximately "+annual_sSSY_disturbed_LOWER_QUARRY_4+" tons/km2/yr. The quarry surfaces are comprised of haul roads, piles of overburden, and steep rock faces which can be described as a mix of unpaved roads and cut-slopes. Literature values show measured sSSY from cutslopes varying from 0.01 tons/km2 /yr in Idaho (Megahan, 1980) to 105,000 tons/km2/yr in Papua New Guinea (Blong and Humphreys, 1982), so the sSSY ranges measured in this study are well within the ranges found in the literature.")
