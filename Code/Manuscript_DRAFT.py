...
document.add_paragraph("A nested-watershed approach was used to quantify the contribution of undisturbed and human-disturbed areas to the SSY to Faga'alu Bay during baseflow and during storm events of varying magnitude (SSYEV). While steep, mountainous watersheds can discharge large amounts of bedload (Milliman and Syvitski, 1992), this research is focused on sediment size fractions that can be transported in suspension in the marine environment to settle on corals, and this is generally restricted to silt and clay fractions (<16um) (Asselman, 2000). The suspended sediment load was calculated at three sampling points in Faga'alu stream that drain key land covers suspected of having different SSY: undisturbed forest (FG1), and disturbed areas including the quarry (FG2) and the village (FG3). Suspended sediment concentrations (SSC) in stream water samples at FG1, FG2, and FG3 were also examined for key differences between undisturbed and disturbed areas, using boxplots and water discharge-sediment concentration (Q-SSC) relationships. ")

## Calculating SSYEV
document.add_paragraph("SSYEV is calculated by integrating continuous estimates of suspended sediment yield, calculated from measured or modeled water discharge (Q) and measured or modeled suspended sediment concentration (SSC) (Duvert et al., 2012):")
add_equation(SSYEV_eq) ## Equation
## Storm Events
document.add_paragraph("Storm events can be defined by precipitation (Hicks, 1990) or discharge parameters (Duvert et al., 2012), and the method used to separate storm events on the hydrograph can significantly influence the analysis of SSYEV (Gellis, 2013). Complex graphical or rule-based techniques for hydrograph separation may be implemented (Dunne and Leopold, 1978; Perrault, 2010), but for this research the simple stage height threshold rule was used due to the flashy hydrologic response, low baseflow discharge, and short duration of recession curves between events (Fahey et al., 2003; Lewis et al., 2001). A storm event was defined as the period of time when stream stage height exceeds a given threshold. In this case, the threshold was the mean stage plus one standard deviation. Complex storm events occurred when subsequent rain fell before the stream stage fell below the storm threshold. These events were separated manually into individual storms for analysis, but required the discretion of the analyst (Duvert, 2012). Where Q peaks were separated by at least a two hour period and Q was nearly at baseflow, complex storm events were separated into individual storm events.")
...
document.add_paragraph("A main objective for this study was to determine how much human disturbance has increased total SSY to Faga'alu Bay (SSYTOTAL). Two approaches were used to determine relative contributions to SSY at the watershed outlet originating from undisturbed and human-disturbed subwatersheds: 1) comparing percent contributions of subwatersheds to the total sediment load, and 2) the Disturbance Ratio (DR).")

document.add_paragraph("The percent contributions of each subwatershed were calculated for each storm event and averaged to determine the relative contributions from the UPPER and LOWER subwatersheds to total SSY. SSY from the UPPER subwatershed was measured at FG1 (SSYUPPER = SSYFG1), SSY at the outlet was measured at FG3 (SSYTOTAL = SSYFG3), and SSY from the LOWER subwatershed (SSYLOWER) was calculated by subtracting SSYFG1 from SSYFG3 (SSYLOWER = SSYFG3-SSYFG1) (Figure "+Study_Area_map['fig_num']+"). Where SSYEV data at FG2 were also available, the contributions from the quarry subwatershed (SSYLOWER_QUARRY = SSY_FG2-SSY_FG1), and village subwatershed (SSYLOWER_VILLAGE = SSY_FG3-SSY_FG2) could be calculated.") 
...
document.add_paragraph("Data on precipitation, water discharge, suspended sediment concentration and turbidity were collected during three field campaigns: January-March, 2012, February-July 2013, and January-March 2014, and several intervening periods of unattended instrument monitoring. Field sampling campaigns were scheduled to coincide with the period of most frequent storms in the November-April wet season, though large storms were sampled throughout the year.")
...
document.add_paragraph("Water discharge (Q) in Faga'alu Stream was calculated from 15 min interval stream stage measurements, using a stage-Q rating curve calibrated to manual Q measurements made under baseflow and stormflow conditions (Figures "+LBJ_StageDischarge['fig_num']+" and "+DAM_StageDischarge['fig_num']+"). Q was measured in the field by the area-velocity method (AV) using a Marsh-McBirney flowmeter to measure flow velocity, and simultaneous channel surveys to measure cross-sectional area (Harrelson et al., 1994; Turnipseed and Sauer, 2010). Stream stage was measured with non-vented pressure transducers (PT) (Solinst Levelogger or Onset HOBO Water Level Logger) installed in stilling wells at two locations in Faga'alu: FG1 and FG3. Barometric pressure data collected at Wx were used to calculate stage from the pressure data recorded by the PT's. Data gaps in barometric pressure were filled by data from stations at Pago Pago Harbor (NSTP6) and NOAA Climate Observatory at Tula (TULA). Priority was given to the station closest to the watershed with valid barometric pressure data. Barometric data were highly correlated and the source data made little (<1cm) difference in the resulting water level.")
...
document.add_paragraph("Water discharge at FG2 was calculated as the product of the specific water discharge from FG1 (Q m3/0.9 km2) and the watershed area draining to FG2 (1.17 km2). This assumes that specific water discharge from the subwatershed above FG2 is similar to above FG1 and rainfall does not vary over these areas. Discharge may be higher from the quarry surface, which represents "+"%.1f"%landcover_table.ix[1]['% Bare Land']+"% of the LOWER_QUARRY subwatershed, so water discharge, and thus sediment loads at the quarry are a conservative, lower bound estimate. The quarry surface is continually being disturbed, sometimes with large pits excavated and refilled in the course of a few weeks, as well as intentional water control structures being implemented over time, so efforts to model water yield from the quarry were considered too time-consuming and uncertain to be useful.")

document.add_paragraph("The model errors (RMSE) for the stage-Q rating curve were "+"%.0f"%LBJ_Man_rmse+"% at FG3 using Manning's equation, and "+"%.0f"%DAM_HEC_rmse+" % at FG1 using HEC-RAS. The RMSE for Q at FG1 was also applied to Q at FG2 because the specific water yield at FG1 is used to predict Q at FG2. The measurement error (RMSE) for Q measurements (same for both FG1 and FG3) from the DUET-H/WQ LUT (Harmel et al., 2006) was "+"%.1f"%AV_Q_measurement_RMSE+" % overall, and was used to calculate the total Probable Error. This measurement error accounted for error in the area-velocity measurements (6%), continuous Q measurement in a natural channel (6%), pressure transducer error (0.1%), and streambed condition (firm, stable bed=0%).")
...
document.add_paragraph("In contrast to other methods like traditional sediment rating curve or the traditional sediment budget, event-wise correlation of SSY and storm metrics was advantageous in small watersheds with hysteresis in Q-SSC relationship.")
