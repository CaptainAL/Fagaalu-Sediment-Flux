...
document.add_paragraph("The CampbellSci OBS500 measures both backscatter(BS) and sidescatter (SS), and both the BS-SSC and SS-SSC relationships at FG3 showed high R2 values (Figure "+LBJ_OBSa_Rating_Curve['fig_num']+" and Figure "+LBJ_OBSb_Rating_Curve['fig_num']+"). Mean SS was used since it is measured the same as NTU measured by the YSI turbidimeter (Anderson 2005).") 
...
document.add_paragraph("Precipitation metrics (Psum and EI30) showed lower correlation coefficients with SSYEV compared to the discharge metrics (Qsum and Qmax) over the full range of storm sizes. SSYEV is calculated from measured Q so it is expected that discharge metrics are more closely correlated, and has been observed in other studies (Rankl, Duvert etc.). EI30 was the least correlated with SSYEV of the storm metrics. No statistically significant correlations (p<0.001) were found between SSY and EI30 for the UPPER subwatershed using Pearson's or Spearman's coefficients, and the statistically significant correlations between SSY and EI30 for the total watershed were the lowest of all storm metrics. ")
