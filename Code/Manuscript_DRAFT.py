...
document.add_paragraph("A main objective for this study was to determine how much human disturbance has increased Total SSY to Faga'alu Bay (SSYTOTAL). Two approaches were used to determine relative contributions to SSYTOTAL from the undisturbed and human-disturbed areas: comparing individual event and average percent contributions, and the Disturbance Ratio (DR)(Eq. "+DR_eq.eq_num+"). SSYTOTAL was measured at FG3, SSY from the UPPER subwatershed (SSYUPPER) was measured at FG1, and SSY from the LOWER subwatershed (SSYLOWER) was calculated by subtracting SSYUPPER from SSYTOTAL (SSYLOWER = SSYFG3 - SSYFG1) (Table "+S_Diff_table.table_num+").")
...
Percent_Upper_S = S_Diff_table['% UPPER']['Total/Avg:']
...
SSY_UPPER, sSSY_UPPER = S_Diff_table['UPPER tons']['Total/Avg:'], S_Diff_table['UPPER tons']['Tons/km2'] 
SSY_LOWER, sSSY_LOWER = S_Diff_table['LOWER tons']['Total/Avg:'], S_Diff_table['LOWER tons']['Tons/km2']
SSY_TOTAL, sSSY_TOTAL = S_Diff_table['TOTAL tons']['Total/Avg:'], S_Diff_table['TOTAL tons']['Tons/km2']
P_measured = S_Diff_table['Precip (mm)']['Total/Avg:']
disturbed_area_LOWER = Area_LOWER * float(S_Diff_table['LOWER tons']['fraction disturbed (%)'])/100

document.add_paragraph("The UPPER and LOWER subwatersheds are similar in size ("+"%0.2f"%Area_UPPER+" km2 and "+"%0.2f"%Area_LOWER+" km2) so assuming the specific SSY (sSSY (tons/km2)) is similar in both watersheds under pre-disturbance conditions, they should account for roughly the same percentage contribution to SSYTOTAL. However, for the "+"%.0f"%len(S_Diff_table)+" storms with valid data from both FG1 and FG3, SSYUPPER accounted for "+Percent_Upper_S_min+"-"+Percent_Upper_S_max +"%, average of "+Percent_Upper_S+"% of SSYTOTAL. SSYLOWER accounted for "+Percent_Lower_S_min+"-"+Percent_Lower_S_max +"%, average of "+Percent_Lower_S+"% of SSYTOTAL (Table "+S_Diff_table.table_num+"). Total SSY from the UPPER and LOWER subwatersheds for the measured storms was "+SSY_UPPER+" and "+SSY_LOWER+" (tons), respectively, and corresponding sSSY was "+sSSY_UPPER+" and "+sSSY_LOWER+" (tons/km2), respectively. Comparing sSSY from the undisturbed UPPER watershed and the disturbed LOWER watershed shows sSSY has been increased by "+S_Diff_table['LOWER tons']['sSSY:sSSY_UPPER']+"x in the LOWER subwatershed, and "+S_Diff_table['TOTAL tons']['sSSY:sSSY_UPPER']+"x for the TOTAL watershed.")

document.add_paragraph("The measured sSSY from the undisturbed UPPER watershed ("+sSSY_UPPER+" tons/km2) was used to estimate SSY from undisturbed areas in the LOWER subwatershed ("+S_Diff_table['LOWER tons']['SSY from forested areas (tons)']+" tons), and estimate SSY from the "+"%0.3f"%disturbed_area_LOWER+" km2 of disturbed areas in the LOWER subwatershed ("+S_Diff_table['LOWER tons']['SSY from disturbed areas (tons)']+" tons). For the measured storms, roughly "+S_Diff_table['LOWER tons']['% SSY from disturbed areas']+"% of SSY from the LOWER subwatershed is from disturbed areas, despite the disturbed areas only accounting for "+S_Diff_table['LOWER tons']['fraction disturbed (%)']+"% of the subwatershed area. Similarly, despite only "+S_Diff_table['TOTAL tons']['fraction disturbed (%)']+"% of the TOTAL watershed being disturbed, SSY from disturbed areas accounts for "+S_Diff_table['TOTAL tons']['% SSY from disturbed areas']+"% of the SSY from the TOTAL watershed. The sSSY from disturbed areas is the same as sSSY from the whole UPPER subwatershed since the disturbed areas cover such a small fraction, their influence is not detected. Estimated sSSY from disturbed areas in the LOWER subwatershed is "+S_Diff_table['LOWER tons']['sSSY from disturbed areas (tons/km2)']+" tons/km2, suggesting human disturbance has increased sSSY by "+S_Diff_table['LOWER tons']['DR for sSSY from disturbed areas']+"x over undisturbed, forest conditions.")

def times(x,factor,round_to):
    return int(int(round(int(factor*float(x))/float(round_to)))* float(round_to))

## comparing sSSY from UPPER subwatershed with other similar watersheds
annual_SSY_UPPER_2, annual_sSSY_UPPER_2 = "%.0f"%times(SSY_UPPER,3,10)+"-"+"%.0f"%times(SSY_UPPER,4,10), "%.0f"%times(sSSY_UPPER,3,10)+"-"+"%.0f"%times(sSSY_UPPER,4,10)
annual_SSY_TOTAL_2, annual_sSSY_TOTAL_2 ="%.0f"%times(SSY_TOTAL,3,10)+"-"+"%.0f"%times(SSY_TOTAL,4,10), "%.0f"%times(sSSY_TOTAL,3,10)+"-"+"%.0f"%times(sSSY_TOTAL,4,10)


## Comparing QUARRY and VILLAGE separately
Percent_UPPER_S = S_Diff_table_quarry['% UPPER']['Total/Avg:']
Percent_QUARRY_S = S_Diff_table_quarry['% LOWER_QUARRY']['Total/Avg:']
Percent_VILLAGE_S = S_Diff_table_quarry['% LOWER_VILLAGE']['Total/Avg:']

SSY_UPPER, sSSY_UPPER = S_Diff_table_quarry['UPPER tons']['Total/Avg:'], S_Diff_table_quarry['UPPER tons']['Tons/km2'] 
...
SSY_TOTAL, sSSY_TOTAL = S_Diff_table_quarry['TOTAL tons']['Total/Avg:'], S_Diff_table_quarry['TOTAL tons']['Tons/km2']
P_measured = S_Diff_table_quarry['Precip (mm)']['Total/Avg:']
...
document.add_paragraph("Annual estimates of SSY and sSSY are most commonly used to compare watersheds, however, a continuous annual time-series of SSY was not possible at the study site due to the discontinuous field sampling trips. For storms totaling "+P_measured+" mm of precipitation meausured in Table "+S_Diff_table.table_num+", SSY from the UPPER and LOWER subwatersheds was "+SSY_UPPER+" and "+SSY_LOWER+" tons, respectively, and sSSY from the UPPER and LOWER subwatersheds was "+sSSY_UPPER+" and "+sSSY_LOWER+" tons/km2, respectively. Annual precipitation is roughly 3-4 times the precipitation measured during storms (on the order of 4,000 mm), so annual SSY and sSSY are estimated to be roughly 3-4 times the measured SSY and sSSY: "+annual_SSY_UPPER_2+" tons/year, and  "+annual_sSSY_UPPER_2+" tons/km2/year for the UPPER subwatershed, and "+"%.0f"%times(SSY_LOWER,3,10)+"-"+"%.0f"%times(SSY_LOWER,4,10)+" tons/year, and  "+"%.0f"%times(sSSY_LOWER,3,10)+"-"+"%.0f"%times(sSSY_LOWER,4,10)+" tons/km2/year for the UPPER subwatershed. SSY and sSSY from the TOTAL watershed was "+SSY_TOTAL+" tons and "+sSSY_TOTAL+" tons/km2, respectively, so annual SSY and sSSY are estimated to be "+annual_SSY_TOTAL_2+" tons/year, and  "+annual_sSSY_TOTAL_2+" tons/km2/year.") 

## ## Annual SSY and sSSY from Table 3 (UPPER and LOWER_QUARRY, LOWER_VILLAGE)
document.add_paragraph("For the storms totaling "+P_measured+" mm of precipitation measured in Table "+S_Diff_table_quarry.table_num+", SSY from the UPPER, LOWER_QUARRY, and LOWER_VILLAGE subwatersheds was "+SSY_UPPER+", "+SSY_LOWER_QUARRY+", and "+SSY_LOWER_VILLAGE+" tons, respectively, and sSSY from the UPPER, LOWER_QUARRY, and LOWER_VILLAGE subwatersheds was "+sSSY_UPPER+", "+sSSY_LOWER_QUARRY+", and "+sSSY_LOWER_VILLAGE+" tons/km2, respectively. Annual precipitation is roughly 6-8 times the precipitation measured for these storms (on the order of 4,000 mm), so annual SSY and sSSY are estimated to be roughly 6-8 times the measured SSY and sSSY. Annual SSY from the UPPER, LOWER_QUARRY, and LOWER_VILLAGE subwatersheds was estimated to be "+annual_SSY_UPPER_3+", "+"%.0f"%times(SSY_LOWER_QUARRY,6,10)+"-"+"%.0f"%times(SSY_LOWER_QUARRY,8,10)+", and "+"%.0f"%times(SSY_LOWER_VILLAGE,6,10)+"-"+"%.0f"%times(SSY_LOWER_VILLAGE,8,10)+" tons/year, respectively. Annual sSSY from the UPPER, LOWER_QUARRY, and LOWER_VILLAGE subwatersheds was estimated to be "+annual_sSSY_UPPER_3+", "+"%.0f"%times(sSSY_LOWER_QUARRY,6,10)+"-"+"%.0f"%times(sSSY_LOWER_QUARRY,8,10)+", and "+"%.0f"%times(sSSY_LOWER_VILLAGE,6,10)+"-"+"%.0f"%times(sSSY_LOWER_VILLAGE,8,10)+" tons/km2/year, respectively. SSY and sSSY from the TOTAL watershed was "+SSY_TOTAL+" tons and "+sSSY_TOTAL+" tons/km2, respectively, so annual SSY and sSSY are estimated to be "+annual_SSY_TOTAL_3+" tons/year, and "+annual_sSSY_TOTAL_3+" tons/km2/year. sSSY from the disturbed fraction ("+S_Diff_table_quarry['LOWER_QUARRY tons']['fraction disturbed (%)']+"%) of the LOWER_QUARRY subwatershed ("+"%0.2f"%Area_LOWER_QUARRY+" km2) was "+S_Diff_table_quarry['LOWER_QUARRY tons']['sSSY from disturbed areas (tons/km2)']+" tons/km2 for the "+P_measured+" mm of precipitation during the measured storms (roughly 12-15% of annual precipitation). Annual precipitation is on the order of 4,000 mm, so annual specific SSY is roughly 6-8 times the measured specific SSY, approximately "+"%.0f"%times(S_Diff_table_quarry['LOWER_QUARRY tons']['sSSY from disturbed areas (tons/km2)'],6,100)+"-"+"%.0f"%times(S_Diff_table_quarry['LOWER_QUARRY tons']['sSSY from disturbed areas (tons/km2)'],8,100)+" tons/km2/year. Ramos-Scharron (2005) tabulated sSSY rates for cut-slopes, finding........")
...
## compare with other wathersheds
...
## Disturbance Ratio
SSYspec_Forest = Q_S_Diff_summary_table.ix['Spec SSY (Mg/km2)'][1]
SSYspec_Village = Q_S_Diff_summary_table.ix['Spec SSY (Mg/km2)'][2]
DR_S = Q_S_Diff_summary_table.ix['SSY Disturbance Ratio'][2]
DR_Q = Q_S_Diff_summary_table.ix['Q Disturbance Ratio'][2]
document.add_paragraph("The DR is the ratio of SSY from the total watershed under current human-disturbed conditions to SSY under pre-disturbance conditions (SSYPRE)(Equation "+DR_eq.eq_num +"). This assumes that prior to humans altering the land cover the whole watershed was covered in forest, and the specific SSY of the whole watershed (tons/km2) was the same as the forest area currently covering the Upper subwatershed. The specific SSY for both FOREST and VILLAGE were calculated by summing all storms from Table "+S_Diff_table.table_num+" and dividing by the subwatershed area. The DR for SSY was "+DR_S+", meaning human disturbance has increased Total SSY "+DR_S+"x over undisturbed levels.") 
